...
Query title convention (required): "<strain>__<full_node_or_contig>__<region>__<alias>__<gene>".
Legacy three-field titles are refused before output writes. Existing alias-only rollups require explicit identity migration into a new output root.
...
REPO_HDR = ["strain","full_node_or_contig","region","bgc_id","exact_locus_identity","gene","aa_length","role","domains","hit_rank",
...
def query_identity(title):
    """Require explicit strain, full contig, region, alias and gene in the query title."""
    parts = title.split("__")
    if len(parts) != 5:
        raise ValueError("QUERY_REQUIRES_STRAIN_FULL_CONTIG_REGION_ALIAS_GENE")
    strain, node, region, alias, gene = parts
    display = exact_locus_display(strain, node, region, alias)
    if any(x != x.strip() for x in parts) or not gene or any(c in gene for c in "\r\n\t"):
        raise ValueError("INVALID_QUERY_COMPONENT")
    if any(x in {".", ".."} or any(c in x for c in "/\\\t") for x in parts[:4]):
        raise ValueError("UNSAFE_LOCUS_COMPONENT")
    locator = "__".join(quote(x, safe=".-_") for x in parts[:4])
    return strain, locator, gene, dict(strain=strain, full_node_or_contig=node,
        region=region, bgc_id=alias, exact_locus_identity=display)


def validate_existing_row(row):
    display = exact_locus_display(row.get("strain"), row.get("full_node_or_contig"),
                                  row.get("region"), row.get("bgc_id"))
    if row.get("exact_locus_identity") != display or not row.get("gene"):
        raise ValueError("EXISTING_ROLLUP_IDENTITY_CONFLICT")
    return display
    ...
    if os.path.exists(path):
        with open(path, newline='') as f:
            for row in csv.DictReader(f):
                validate_existing_row(row)
                genes.setdefault(row.get('gene',''), []).append(row)
    return genes

def load_existing_roll(path):
    """Rollup rows already on disk, keyed (bgc_id, gene) — the rollup's analogue of
    `load_existing`, which keys by gene alone because a repository file is already one BGC.

    v9.7.416. The rollup writer's own comment called this file "dated, additive" but it opened with
    mode "w" and wrote only the current batch, so a second ingest on the same day replaced the
    first instead of adding to it. The repository block three lines above always merged. Rollups
    are what `deliverable_tools/roster_v2.py` globs (`blastp_nr_*/*_nr_top_hit_per_gene_*.csv`,
    and `all_strains()` derives the roster itself from that glob), so a truncated rollup silently
    narrows per-gene nr evidence downstream. Earlier DATES were never at risk: they are separate
    directories.
    """
    out = OrderedDict()
    if os.path.exists(path):
        with open(path, newline='') as f:
            for row in csv.DictReader(f):
                out.setdefault((validate_existing_row(row), row.get('gene','')), []).append(row)
    return out
...
def main():
    ...
    seen = {}; report=[]; ingested_ids=[]; identities={}
    for base in ids:
        ...
        with open(csvp, newline='') as f:
            for cols in csv.reader(f):
                ...
                strain,bgc,gene,identity=query_identity(cols[0])
                identities[(strain,bgc)] = identity
        ...
        for (strain,bgc,gene),hits in per.items():
            ...
            for rank,h in enumerate(hits[:10],1):
                rows.append(OrderedDict([
                    ("strain",strain),("full_node_or_contig",identities[(strain,bgc)]["full_node_or_contig"]),
                    ("region",identities[(strain,bgc)]["region"]),("bgc_id",identities[(strain,bgc)]["bgc_id"]),
                    ("exact_locus_identity",identities[(strain,bgc)]["exact_locus_identity"]),("gene",gene),
                    ("aa_length",""),("role",""),("domains",""),
                    ("hit_rank",str(rank)),("subject_acc",h["subject_acc"]),
                    ("subject_organism",h["subject_organism"]),("subject_def",h["subject_def"]),
                    ("pct_identity",h["pct_identity"]),("align_length",h["align_length"]),("query_coverage",""),
    ...
    # Validate every existing target before opening any output for writing.
    for (strain, bgc), genes in newrepo.items():
        for template in (REPO_TOP10[channel], REPO_TOPHIT[channel]):
            load_existing(os.path.join(a.repo, strain, bgc, template.format(bgc=bgc)))
        directory = os.path.join(a.master, strain, f"blastp_{channel}_{a.date}")
        for suffix in ("top10", "top_hit_per_gene"):
            load_existing_roll(os.path.join(directory, f"{strain}_{channel}_{suffix}_{a.date}.csv"))
    ...
    # ---- write master rollup (dated, MERGED — see load_existing_roll) ----
    ...
    for strain,items in sorted(bystrain.items()):
        ...
        # Build this run's rollup rows keyed exactly like the repository block above, then overlay
        # them onto whatever the same dated file already holds. Without the overlay a second ingest
        # on the SAME DAY for the same strain+channel truncated the file to just that batch.
        fresh10, freshth = OrderedDict(), OrderedDict()
        for bgc,g,rows in items:
            for i,row in enumerate(rows):
                out={k:row.get(k,"") for k in REPO_HDR}
                # HSP length is already in align_length; query length is unknown.
                out["channel"]="ncbi_nr" if channel=="nr" else channel
                out["provenance"]=f"_raw/{seen[(strain,bgc,g)]}-Alignment-HitTable.csv"
                fresh10.setdefault((out["exact_locus_identity"],g),[]).append(out)
                if i==0: freshth[(out["exact_locus_identity"],g)]=[out]
        for path,fresh in ((p10,fresh10),(pth,freshth)):
            merged=load_existing_roll(path)
            merged.update(fresh)                       # same gene -> replaced; siblings untouched
            with open(path,"w",newline='') as fh:
                w=_SafeDictWriter(fh,fieldnames=ROLL_HDR); w.writeheader()
                for key in sorted(merged):
                    for row in merged[key]:
                        w.writerow({k:row.get(k,"") for k in ROLL_HDR})
    ...
    with open(logp,"a") as f:
        ...
        for (strain,bgc),genes in sorted(newrepo.items()):
            f.write(f"- {identities[(strain,bgc)]['exact_locus_identity']}: {' '.join(genes)}\n")
    emit(f"log -> {logp}")
