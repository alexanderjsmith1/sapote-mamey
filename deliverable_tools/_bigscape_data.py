...
Stdlib + one intra-bundle import (mamey.bigscape_namespace.strain_from_gbk_name, the single
strain resolver; unresolved identity or import errors fail closed). No network, no external assets.
...
# prior organism-field behaviour.
_bundle_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if _bundle_root not in _sys.path:
    _sys.path.insert(0, _bundle_root)
from mamey.bigscape_namespace import strain_from_gbk_name as _canonical_strain
...
def strain_of(path: str, organism: str):
    ...
    # .417 fail-closed rewrite then made the terminal `raise REFERENCE_STRAIN_UNBOUND` fire on every
    # one, so gbk_index (and the matrix + clinker widgets) crashed on any MIBiG-anchored DB. Classify
    # MIBiG explicitly, the same basename-BGC test the four BiG-SCAPE->Mamey tools use; the
    # fail-closed raise stays for genuinely-unbound NON-MIBiG references.
    if re.fullmatch(r"BGC\d{7}(?:\.\d+)?\.(?:gbk|gb|gbff)", b, re.I):
        return "MIBiG", "MIBiG"
    m = re.match(r"(AS-\d+)", b)
    if m:
        return m.group(1), "AS"
    ...
    # an accession fragment into the label (e.g. "Micromonospora sp. WMMA1998_CP11..."). Route the
    # reference label through strain_from_gbk_name -- the single resolver the four BiG-SCAPE->Mamey
    # tools use (v9.7.412/.416) -- which keeps the full strain from the filename BiG-SCAPE clustered.
    if _canonical_strain is not None:
        cs = _canonical_strain(b)
        if cs and cs != "MIBiG":
            return cs, "Type"
    raise ValueError("REFERENCE_STRAIN_UNBOUND")
