...
def summed_bitscores_vs_ref(assembly_fa, ref_fa, blastn, makeblastdb, tmp):
    ...
    dbp = os.path.join(tmp, os.path.basename(ref_fa) + ".db")
    subprocess.run([makeblastdb, "-in", ref_fa, "-dbtype", "nucl", "-out", dbp],
    ...
    proc = subprocess.run([blastn, "-query", assembly_fa, "-db", dbp, "-task", "megablast",
                           ...
                           "-max_target_seqs", "5"], capture_output=True, text=True)
