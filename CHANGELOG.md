...
Accretion-justified: no new mamey runtime module. New standalone track adapter and R display helpers bridge the existing admitted Figure Factory contract.

- **Bundle-pinned execution and identity refusal.** Launch child commands through the bundled runner; detect stale engine or bundle-version bytecode; report mismatched child engine receipts before trusting intake output.
- **Portable placement assets and output containment.** Resolve workspace assets through explicit roots or verified marker parents, diagnose missing companion binaries, accept documented flat placement layouts, normalize merged FASTA deflines, and refuse reference-package output inside the bundle, including symlink routes.
- **Admitted assay tracks on existing trees.** Join exact tip crosswalks to hash-bound Figure Factory selections. Preserve zero, negative and unmeasured values; support configurable display thresholds and explicit reference labels. Ship generic fixtures and single/grouped R renderers, with no project-specific pooling or taxonomy inference.
- **Re-authored clinker and cohort controls.** Bound long-label layout, deterministic row order, visible hidden-track counts, valid query regexes, full deposited labels and centered PDFs. Add opt-in product parking and explicit query-only CDS-count staging filters with provenance receipts; preserve reference inputs and join keys.
- **Companion setup and documentation.** Prepend the active interpreter bin for BiG-SCAPE subprocesses; document optional R dependencies, provide installer readiness checks and an operator-supplied `--from <dir>` offline route (no third-party package source is vendored); regenerate inventories and observed health metrics.

Engine remains 1.9.169. No scoring threshold, scientific acceptance, publication authority or owner seal is changed. A local engineering cut candidate requires its own source and extracted-archive validation; consult RELEASE_MANIFEST.md for the measured status.
...
- **Gene-cluster family figure:** `tools/render_gcf_synteny_tree.py` draws a BiG-SCAPE family tree beside role-coloured gene-arrow tracks anchored on the first ketosynthase, with dotted homology links; row labels carry the four-component BGC identity from the package inventory or an explicit identity hold.
- **Tree build and placement fixes:** GToTree 2 native invocation, HMM path and exit-status masking; build completion gate requires `.contree`, supports and the IQ-TREE wall-clock line; the query-tip SCG filter fails loudly; outgroup registry rows that name an accession write a single-accession header that passes the placement gate; non-type references are no longer classified as outgroup; the report gate ignores the ingroup stem after display rooting; the reference-metadata resolver checks BioSample, fails per row instead of silently, and sets the TLS certificate note; 16S database provenance and genus-string defects; panel labels come from deposit titles and `[Type]` only from a verified type-material check; assay columns are bound only from the owner table; omitted-tip receipts carry a reason; renderer captions count tips, not inference genomes.
- **Engine 1.9.166 (BUMPED — score-neutral; boards from v9.7.431 remain poolable):** `SM_HMM_DB` / `MAMEY_HMM_DIR` / `$MAMEY_DATA_ROOT/hmm` resolve through one shared precedence and `doctor` reports what the scanner will read; `published_registry` distinguishes "not configured" from "configured and empty" so an empty roster no longer fails open (enabling change, no caller yet); a BiG-SCAPE region name carrying a variant suffix between the strain prefix and `_NODE_` resolves to its own visible id instead of merging into the canonical strain; `ingest-blastp-trove` gains the explicit `--rekey-by-locus` migration mode described below (reader-side, non-scoring).
- **BLASTp trove re-key for Mode B:** troves whose folder and row BGC aliases predate the selected package used to quarantine almost every row as `NONCURRENT_LOCUS`, so the Mode B BLASTp gate could never clear. `ingest-blastp-trove --rekey-by-locus` resolves each query locus to its current BGC through the package's sole `*cds_table.csv` and writes to that BGC; absent, ambiguous and length-conflict loci stay quarantined as distinct typed holds; the strict default path is unchanged; the receipt schema is `mamey_blastp_locus_rekey_ingest_v2` and each overlay row records `source_query_bgc` and `locus_rekey_state`.
- **Shipped outgroup registry snapshot:** `mamey/data/outgroup_registry.tsv` is a verbatim snapshot of the ruled `OFFICIAL_DATA/OUTGROUP_REGISTRY.tsv` (workspace paths in rationale text redacted). Owner ruling: the organism names stand, so the Micromonospora and Salinispora rows now carry the Actinoplanes missouriensis 431 type-material genome (GCF_000284295.1) and the Pseudonocardia, Amycolatopsis and Saccharomonospora rows the Actinosynnema mirum DSM 43827 genome (GCF_000023245.1), replacing accessions that resolve at NCBI to other organisms. The shipped-vs-ruled guard is now a plain test that fails the moment the ruled table moves ahead of the snapshot.
- **Retired:** `tools/gtotree_panel_render.R` and its two renderer tests; `tools/render_tree_COLOR_STRIPS.R` is the only GToTree panel renderer.
- **Housekeeping from the patch queue:** `docs/EXTERNAL_TOOL_INVENTORY.md` package-estate row read from the package manifests; the PDF-rendering route recorded in AGENTS.md; `tools/emit_modeb_template_full50.py` supplies the 50-section Mode B template; the field-collection test fixture uses verified-synthetic identifiers; the five `docs/reference` version-of-record lines are now owned by `sync_version.py`; the placement renderer caption is generic and truthful; `docs/FIGURES_START_HERE.md` names the tree and family renderers; the print-call ratchet stays at the signed observed count (1324) because the four new tool entry points are paid for by merging consecutive single-line emits in `mamey/cli.py`, `mamey/package_inspector.py` and `tools/phylo_roster.py` (output byte-identical) and by dropping a per-pair progress line in the family figure tool; `tools/MANIFEST.json` regenerated without cache files.
- **Not in this cut (needs a ruling or another lane):** the redox-cofactor marker inversion and the `*_domains.csv` description column (two engine cards assigned to Codex for review); the ARTS gene-list silent drop (another lane); the lab_quest release-class de-duplication (must follow roster population); the antiSMASH version stamp at intake; geography for the ten attine query strains.
