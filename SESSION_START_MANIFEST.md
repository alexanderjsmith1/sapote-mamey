# SESSION START MANIFEST — Sapote–Mamey
**Read this FIRST, at the start of every session.** It is the menu of what the bundle can do, so options (tools, modes, deliverables) are known up front instead of being rediscovered mid-task. Engine: Mamey v1.9.152 · Bundle: sapote-mamey-v9.7.414.
**Before reporting any capability as unavailable, read `COMPANION_FILES.md`** — Sapote–Mamey always travels with companion files (genome FASTAs, antiSMASH ZIPs, MIBiG references, an optional DIAMOND binary) that are attached separately in the chat, not packed in this zip. If one is missing, name it and ask the user to attach it; assume "forgot to attach" before "doesn't exist." (The optional alignment/add-on wheels are **not** packed in this zip — they ship separately and are installed via `install_sapote_addons.sh`; without them the add-on stack is unavailable, so name the installer and ask the user to attach the wheels rather than reporting the stack as broken.)
Companions: `000_READ_ME_FIRST_CHATGPT_CLAUDE.md`, `CHATGPT_START_HERE.md`, `CLAUDE_START_HERE.md`, `CHATGPT_READ_ME_FIRST.md`, optional `CHATGTP_READ_ME_FIRST.md`, `README_START_HERE.md` (run basics), `docs/WORKFLOW_GUIDE.md`, `docs/DELIVERABLE_CONTRACT.md`, `docs/CHATGPT_EXECUTION_SLICE_v97147.md` (default ChatGPT/Sapote execution controller), `docs/SAPOTE_SLIM_JUDGMENT_KERNEL.md` (legacy only).
...
Claude↔Codex handoff. Read it before older cohort-specific guides or versioned addenda.

> **v9.7.371 currency pointer:** capabilities added at v9.7.368–.370 (availability step,
> identity guard, asset loop, coverage waves, strictness-aware BiG-SCAPE staging — BREAKING
> `--strictness` — and the `sapote_hooks/` registry) are indexed in `docs/WHATS_NEW_368_370.md`.
> Version literals older than v9.7.370 in the body below are historical mentions.
...
**Directory map:** `mamey/` (engine + `_vendor/ijson`) · `tools/` (309 scripts, §2) · `prompts/` (+`reuse/`, §4) · `docs/` (reference, `modules/`, `troubleshooting/`, `standalone/`, `legacy/`) · `cohort/` · `offline_deps/` · `examples/` · `templates/` · `deliverables/` (+`reports/`).

## 7 · Session-start checklist
1. Read this manifest. 2. Search prior conversation history (CDSW protocol) for the active strain/version. 3. Confirm engine version + which `--json-evidence` mode. 4. Pick the mode/deliverable from §1/§3 that matches the request. 5. **Answer any "what's in the bundle / how many X" question from §6 — don't guess.** 6. Honor §5 constraints throughout. 7. End with exactly 8 unique plain-text numbered next paths, each with a substantive 2–3 sentence description: what it involves, why it matters now, and what it would produce.

## 7b · Session-CLOSE checklist (CDSW close — auto-emit, patch G3)
At the end of every working session, **before** the numbered next-steps, emit the deliverable checklist so the user always sees what is DONE vs available and is warned about data-loss:
```
python tools/session_checklist.py --durable-root-config <roots.json> --durable-root-id <root_id> --scan-relative <relative/path> --receipt-relative <receipts/session-close.json> --registry <intake_registry.csv>
```
It prints a plain-text `[x]/[ ]` checklist of the standard deliverable menu, a persistence / data-loss advisory (working-tree resets are ephemeral; uploaded antiSMASH ZIPs aren't retained; no master workbook unless `--master` ran), and a judgment-PENDING reminder. This is not optional — it is the standard CDSW close artifact.

---

<!-- TOOLS_INVENTORY:START (generated by tools/gen_tools_inventory.py — do not edit) -->
### Tools inventory (309 scripts in `tools/`)
