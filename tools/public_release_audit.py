...
from archive_leak_scan import DEFAULT_ARCHIVE_EXT, scan_archive
...
def _allowance_matches_digest(policy: PublicExportPolicy, identifier_id: str, rel: str,
                              digest: str, candidate: Path) -> bool:
    try:
...
def _allowance_matches(policy: PublicExportPolicy, identifier_id: str, rel: str, text: str, candidate: Path) -> bool:
    return _allowance_matches_digest(
        policy, identifier_id, rel, hashlib.sha256(text.encode("utf-8")).hexdigest(), candidate)


def _archive_provenance(finding: dict, policy: PublicExportPolicy) -> str:
    """Return portable container/member provenance without echoing a declared private literal."""
    result = f"{finding.get('archive', '')}!{finding.get('member', '')}".rstrip("!")
    for identifier in policy.private_identifiers:
        result = result.replace(identifier.literal, f"<private:{identifier.identifier_id}>")
    return result


def _archive_policy_hits(root: Path, policy: PublicExportPolicy) -> list[str]:
    """Scan ZIP-family member paths and text using the same bound policy literals as plain files.

    Raw AS/AJS/PENDING matching remains a separate observation and is deliberately disabled here:
    only the operator-owned privacy profile is authority for this policy pass.  Any read, parse,
    depth, or size truncation is a typed failure because an incomplete release scan is not clean.
    """
    if not policy.private_identifiers:
        return []
    policy_identifiers = tuple(
        (identifier.identifier_id, identifier.literal) for identifier in policy.private_identifiers)
    hits: list[str] = []
    for candidate in sorted(root.rglob("*")):
        # Select lexically before asking the filesystem what the candidate is.  is_file()
        # follows symlinks and returns False for a broken link, which would silently omit the
        # exact archive-shaped path the lstat-based scanner is responsible for rejecting.
        if candidate.suffix.lower() not in DEFAULT_ARCHIVE_EXT:
            continue
        relative = candidate.relative_to(root)
        if _is_excluded_from_scan(relative):
            continue
        rel = relative.as_posix()
        findings = scan_archive(
            candidate, policy_identifiers=policy_identifiers, scan_builtin=False,
            denylist=[], archive_label=rel)
        digest: str | None = None
        for finding in findings:
            provenance = _archive_provenance(finding, policy)
            kind = finding.get("kind", "unknown_archive_scan_error")
            if kind == "policy_private_identifier":
                identifier_id = str(finding.get("identifier_id", finding.get("token", "")))
                if digest is None:
                    try:
                        digest = hashlib.sha256(candidate.read_bytes()).hexdigest()
                    except OSError:
                        digest = ""
                if _allowance_matches_digest(policy, identifier_id, rel, digest, candidate):
                    continue
                hits.append(
                    f"PUBLIC_POLICY_PRIVATE_IDENTIFIER_IN_ARCHIVE_CONTENT: "
                    f"{identifier_id}: {provenance}")
            elif kind == "policy_private_identifier_in_member_path":
                identifier_id = str(finding.get("identifier_id", finding.get("token", "")))
                hits.append(
                    f"PUBLIC_POLICY_PRIVATE_IDENTIFIER_IN_ARCHIVE_MEMBER_PATH: "
                    f"{identifier_id}: {provenance}")
            else:
                hits.append(f"PUBLIC_POLICY_ARCHIVE_SCAN_INCOMPLETE: {kind}: {provenance}")
    return hits
...
def _policy_hits(root: Path, policy: PublicExportPolicy) -> list[str]:
    ...
    hits.extend(_archive_policy_hits(root, policy))
    return hits
...
def audit_surface_report(root: Path, policy: PublicExportPolicy) -> dict[str, int]:
    ...
    counts = {"TEXT_DECODED": 0, "ARCHIVE_POLICY_SCANNED": 0, "BINARY_PATH_ONLY": 0,
              "VENDOR_BINARY_PATH_ONLY": 0, "CACHE_EXCLUDED_BY_BUILDER": 0}
    for candidate in root.rglob("*"):
        ...
        if _is_excluded_from_scan(relative):
            ...
            continue
        if policy.private_identifiers and candidate.suffix.lower() in DEFAULT_ARCHIVE_EXT:
            counts["ARCHIVE_POLICY_SCANNED"] += 1
            continue
        if not candidate.is_file():
            continue
        if candidate.suffix.lower() in _SKIP_SUFFIX:
