...
def _parse(text):
    # v9.7.416: every read of `s` is bounds-checked. Unguarded `s[pos]` turned an EMPTY or truncated
    # tree into `IndexError: string index out of range`, which tells the caller nothing about which
    # file was bad or why. `tools/tree_bgc_overlay.py::parse_newick` already raises a typed
    # ValueError for unsupported Newick; this matches that convention. 89 zero-byte .newick files
    # exist in this workspace (BiG-SCAPE writes one per singleton GCF), so degenerate input is a
    # routine condition, not an edge case.
    ...
    def node():
        ...
        if _at(pos) == '(':
            ...
            while True:
                ...
                if _at(pos) == ')':
                    pos += 1; break
                # Neither ',' nor ')': the old loop re-entered node() on input that never advanced,
                # which is an INFINITE LOOP. Refuse instead of hanging.
