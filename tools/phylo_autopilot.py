...
except ImportError:  # imported as tools.phylo_autopilot
...
def parse_blast_genus(blast_tsv_text: str, conflict_delta: float = 0.5) -> dict:
    """Parse ranked hits and retain a near-tied different-genus conflict.

    The first BLAST row remains the top hit. A different genus within ``conflict_delta`` percentage
    points and at least 95% of the top alignment length prevents automatic tree routing.
    """
    candidates = {}
    for ln in blast_tsv_text.splitlines():
        ...
        try:
            pidf, alnf = float(pid), float(aln)
        except ValueError:
            pidf, alnf = 0.0, 0.0
        candidates.setdefault(qid, []).append(
            {"genus": genus, "pident": pidf, "aln_len": aln, "aln_len_numeric": alnf,
             "title": stitle})
    out = {}
    for qid, hits in candidates.items():
        best = dict(hits[0])
        alternates = []
        for hit in hits[1:]:
            if (hit["genus"] != best["genus"]
                    and best["pident"] - hit["pident"] <= conflict_delta
                    and hit["aln_len_numeric"] >= 0.95 * best["aln_len_numeric"]):
                alternates.append(hit["genus"])
        best.pop("aln_len_numeric", None)
        best["alternate_genera"] = sorted(set(alternates))
        best["routing_state"] = "GENUS_CONFLICT" if alternates else "TOP_HIT_ONLY"
        out[qid] = best
    return out


def assign_genus(query_fasta, db, blastn="blastn", threads=2, runner=subprocess.run) -> dict:
    r = runner([blastn, "-query", query_fasta, "-db", db, "-max_target_seqs", "5", "-max_hsps", "1",
...
def genera_for_reference(assigned: dict) -> dict:
    ...
    for _q, info in assigned.items():
        if info.get("routing_state") == "GENUS_CONFLICT":
            continue
...
def build_reference_fasta(genera, db, out_fasta, blastdbcmd="blastdbcmd", cap_per_genus=30,
    ...
    titles = [(acc, title) for acc, title in titles
              if _p16.reference_definition_hold(title) is None]
...
def write_routing_table(assigned: dict, out_tsv: str) -> None:
    with open(out_tsv, "w") as fh:
        fh.write("query\ttophit_genus\tpident\taln_len\trouting_state\talternate_genera\t"
                 "route_class\tgroup\ttophit_title\n")
        for q in sorted(assigned):
            info = assigned[q]
            conflict = info.get("routing_state") == "GENUS_CONFLICT"
            cls = "GENUS_CONFLICT" if conflict else route_genus(info["genus"])
            grp = ROUTE_TO_GROUP.get(cls, "-")
            fh.write(f"{q}\t{info['genus']}\t{info['pident']:.2f}\t{info['aln_len']}\t"
                     f"{info.get('routing_state', 'TOP_HIT_ONLY')}\t"
                     f"{'|'.join(info.get('alternate_genera', []))}\t"
...
def cmd_route(a):
    ...
    cls = Counter("GENUS_CONFLICT" if i.get("routing_state") == "GENUS_CONFLICT"
                  else route_genus(i["genus"]) for i in assigned.values())
...
def cmd_run_16s(a):
    ...
    query_records = list(_fasta_records(a.query))
    qc_rows = _p16.screen_query_records(query_records, fragment_threshold=a.fragment_threshold)
    qc_path = os.path.join(a.outdir, "query_qc.tsv")
    _p16.write_query_qc_tsv(qc_rows, qc_path)
    held = [row for row in qc_rows if row["admission_state"] == "HOLD"]
    if held:
        print(f"[autopilot] refusing {len(held)} query record(s); inspect {qc_path}", file=sys.stderr)
        return 2
    assigned = assign_genus(a.query, a.db, threads=a.threads)
    ...
    members = {q for q, i in assigned.items()
               if i.get("routing_state") != "GENUS_CONFLICT"
               and ROUTE_TO_GROUP.get(route_genus(i["genus"])) == a.group}
    qc_by_id = {row["query_id"]: row for row in qc_rows}
    missing = sorted(members - set(qc_by_id))
    if missing:
        print(f"[autopilot] routing/query ID mismatch: {missing}", file=sys.stderr)
        return 2
    fragmentary = any(qc_by_id[q]["fragmentary"] for q in members)
...
def build_argparser():
    ...
    x.add_argument("--fragment-threshold", type=int, default=1200, metavar="BP",
                   help="use MAFFT --addfragments when any routed query is shorter than BP "
                        "(default 1200); query_qc.tsv records the decision")
