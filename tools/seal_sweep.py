...
doc surfaces a reader hits first. Two independent checks:
  ...
  2. Version-citation check — every *_START_HERE.md must cite the current version recorded in
     ...
     against the real v9.7.405 sealed tree, it finds docs/FIGURES_START_HERE.md citing no "9.7.405"
...
DOC_SURFACE_GLOBS = ("README*.md", "*_START_HERE.md", "CURRENT_DOCS_INDEX.md", "AGENTS.md", "CLAUDE.md", "docs/FIGURES_START_HERE.md")
    ...
    for pattern in DOC_SURFACE_GLOBS:
        for path in sorted(root.glob(pattern)):
            ...
            for phrase in PROHIBITED_PHRASES:
                if phrase in text:
                    findings.append({"path": path.relative_to(root).as_posix(), "code": "BANNED_PHRASE",
...
def sweep_start_here_version_citation(root):
    ...
    findings = []
    for path in sorted({p for pattern in ("*_START_HERE.md", "AGENTS.md", "CLAUDE.md", "docs/FIGURES_START_HERE.md")
                        for p in root.glob(pattern)}):
        if not path.is_file():
            continue
        text = path.read_text(encoding="utf-8", errors="replace")
        if version not in text:
            findings.append({"path": path.relative_to(root).as_posix(), "code": "VERSION_STALE",
