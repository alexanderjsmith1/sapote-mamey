...
mkdir -p "$OUT"
PYTEST_LOG="${PYTEST_LOG:-$OUT/pytest_full_suite_${STAMP}.log}"
if [[ "$SKIP_TESTS" -eq 0 ]]; then
  say "gate: full suite (captured before manifest generation)"
  if ! PYTHONPATH=. python3 -m pytest tests/ -q -p no:cacheprovider >"$PYTEST_LOG" 2>&1; then
    tail -20 "$PYTEST_LOG" >&2
    die "full suite has failures — fix before cutting."
  fi
else
  say "gate: bind caller-supplied green full-suite log"
  [[ -n "${PYTEST_LOG:-}" && -f "$PYTEST_LOG" ]] \
    || die "--skip-tests requires PYTEST_LOG=/absolute/path/to/a fresh green full-suite log."
fi
