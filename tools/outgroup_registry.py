...
def _shipped_registry_path():
    return str(shipped_registry_path())


def _registry_default():
    return str(outgroup_registry_path())
...
def find_row(genus, scope="genus", path=REGISTRY):
    ...
    """
    binding = registry_binding(path)
    if binding["authority"] == "BUNDLED_REFERENCE_ONLY":
        raise RegistryAuthorityError("OUTGROUP_AUTHORITY_UNBOUND: select a governed external registry; bundled rows are reference only")
    rows = load_registry(path)
    for row in rows:
        row["registry_provenance"] = binding
    g = genus.strip().lower()
    ...
    if exact:
        if len(exact) > 1:
            ...
            if locked:
                identities = {(r["outgroup_genus"], r["outgroup_species_strain"], r["assembly_accession"]) for r in locked}
                if len(identities) > 1:
                    raise RegistryAuthorityError("OUTGROUP_REGISTRY_CONFLICT: multiple incompatible LOCKED rows")
                return locked[0]
...
def main():
    ...
    try:
        return a.func(a)
    except RegistryAuthorityError as e:
        sys.exit(str(e))
