#!/usr/bin/env python3
"""Write qualified KNOWN/NOVEL GCF rows from one explicitly selected BiG-SCAPE run."""
...
def build_rows(db, run_id, cutoff=None, min_strains=2):
    ...
    families = collections.defaultdict(lambda: {
        "strains": set(), "locators": set(), "mibig": set(),
        "products": collections.Counter(), "bin": "",
    })
    assignments = {}
    for raw_cutoff, family_id, bin_label, path, product, category, record_id in source:
        ...
        if selected_cutoff is not None and identity.normalized_cutoff != selected_cutoff:
            continue
        prior = assignments.get((record_id, identity.normalized_cutoff))
        ...
        assignments[(record_id, identity.normalized_cutoff)] = identity.qualified_family_id
        ...
        if base.upper().startswith("BGC"):
            data["mibig"].add(os.path.splitext(base)[0])
        else:
            strain = strain_from_gbk_name(base)
            data["strains"].add(strain)
            data["locators"].add(f"{strain}:{locator(base)}")
    output = []
    for identity, data in families.items():
        ...
        output.append({
            ...
            "status": "KNOWN" if data["mibig"] else "NOVEL",
            "dominant_product": _dominant(data["products"]), "n_mibig": str(len(data["mibig"])),
            "mibig_matches": ";".join(sorted(data["mibig"])),
            "members_locators": ";".join(sorted(data["locators"])),
        })
    return sorted(output, key=lambda row: (
        Decimal(row["normalized_cutoff"]), -int(row["n_strains"]), row["qualified_family_id"],
        row["members_locators"], row["dominant_product"], row["status"],
    ))


def render_rows(rows):
    fields = ["cutoff", "family_id", "run_id", "normalized_cutoff", "qualified_family_id",
              "gcf_namespace", "n_strains", "strains", "status", "dominant_product",
              "n_mibig", "mibig_matches", "members_locators"]
    return "\t".join(fields) + "\n" + "".join(
...
def main(argv=None):
    ...
    parser.add_argument("--db", required=True); parser.add_argument("--out", required=True)
    parser.add_argument("--run-id", required=True); parser.add_argument("--cutoff")
    ...
    emit(f"wrote {len(rows)} qualified known/novel families", file=sys.stderr)
    return 0
