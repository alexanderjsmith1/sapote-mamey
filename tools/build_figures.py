...
def _rescue_pair_count(value, strain):
    """Require a measured nonnegative integer; missing linkage is not zero."""
    import math
    if isinstance(value, bool) or value is None or value == "":
        raise ValueError(f"RESCUE_LINKAGE_UNAVAILABLE: {strain}: missing or invalid pair count")
    try:
        number = float(value)
    except (TypeError, ValueError, OverflowError) as exc:
        raise ValueError(f"RESCUE_LINKAGE_UNAVAILABLE: {strain}: invalid pair count") from exc
    if not math.isfinite(number) or number < 0 or not number.is_integer():
        raise ValueError(f"RESCUE_LINKAGE_UNAVAILABLE: {strain}: invalid pair count")
    return int(number)
...
def _rescue_from_banks(banked):
    ...
    for sid in edge:
        ...
        record = rg.get(sid) if isinstance(rg, dict) else None
        if not isinstance(record, dict) or record.get('status') not in (None, 'PASS', 'NULL_NO_RGGMCI_PAIRS'):
            raise ValueError(f"RESCUE_LINKAGE_UNAVAILABLE: {sid}: linkage bank absent or not usable")
        efls = _rescue_pair_count(record.get('n_pairs'), sid)
        ...
        try:
            if 'Fragment_Rescue_Tiers' in wb.sheetnames:
                ...
                for r in rows[1:]:
                    s=r[ix['strain']]
                    if not s: continue
                    out.append({'strain':s,'efls':_rescue_pair_count(r[ix['EFLS_Pairs']], s),'frag':r[ix['Frag_Loss']] or 0,
...
def plot_rescue(rows, path):
    for row in rows:
        _rescue_pair_count(row.get("efls"), row.get("strain", "unknown"))
