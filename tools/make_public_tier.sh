...
if [ "${SKIP_INTIER_PYTEST:-0}" = "1" ]; then
  ...
  if ! ( cd "$STAGE" && PYTHONPATH="$STAGE" "$PYTHON" -m pytest tests/ -p no:randomly -q >/tmp/_tier_pytest.log 2>&1 ); then
  ...
  echo "WARNING: pytest not available — skipping in-tier test gate (install to enforce)." >&2
fi
