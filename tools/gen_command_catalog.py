...
GROUPS: list[tuple[str, str, tuple[str, ...]]] = [
    ...
    ("Post-seal figures", "Render from a sealed package; never changes a score.",
     ("render-figures", "figure-factory", "figures", "interactive-figures", "cohort-figures", "diagram", "atlas",
      ...
      "render-widgets", "attach-bgc-overlays", "codex-figure-catalog", "codex-figure-sets", "codex-figure-sources",
    ...
    ("Cohort and cross-strain", "Aggregation across sealed packages.",
     ("cohort", "cohort-leads", "cohort-assemble", "cohort-precompute", "majority-read", "novelty-shortlist",
      ...
      "split-overmerge-cards", "compound-families", "p450-tailoring", "assembly-line", "discover", "discovery", "explore",
      "compare", "activity-leads", "activity-lead-genes", "af-ab-appendix", "cddr-pks", "directed-pks-study",
      "wise-fragmented-pks")),
    ("Phylogeny", "Existing-output consumers; no tree is inferred here.", ("phylo-run", "phylo-autopilot", "ani", "signoff")),
    ("Workflow, receipts, catalogs", "Non-destructive status and lookups.",
     ("workflow", "deliverables", "deliverable-queue", "ingest-receipts", "validate-finished-review-request", "list-bgcs", "literature", "lookup", "search",
      "lab-quest",
      "triage-raw", "chatgpt-init", "wheelhouse", "list", "clean", "add-strain")),
]
...
def _subparsers():
    ...
    for action in parser._actions:
        if isinstance(action, argparse._SubParsersAction):
            ...
            return {name: (helps.get(name) or (sub.description or "")) for name, sub in action.choices.items()}
    return {}
...
def render() -> str:
    ...
    if other:
        ...
        for n in other:
            ...
            lines.append(f"| `{n}` | {help_ or '(no description)'} |")
        lines.append("")
    lines += [f"_{len(subs)} commands catalogued._", ""]
    return "\n".join(lines)
