...
def main():
    ...
    emit(str(f'[support] {len(rows)} queries -> {outp}'), str(f'  best-edge LWR: min={lw[0]:.3f} median={lw[len(lw) // 2]:.3f} max={lw[-1]:.3f}; {sum((1 for x in lw if x < 0.5))} of {len(lw)} below 0.5 (mass spread over several edges)'), sep="\n")
