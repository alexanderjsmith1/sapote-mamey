...
def build_af_dossier(recon, min_hit=50.0, only=None):
    ...
    with open(recon) as f:
        for r in _csv.DictReader(f):
            ...
            d = agg.setdefault(s, {"cand": False, "mrsa": False,
            ...
            credible = tier in CREDIBLE and (mx or 0) >= min_hit
            if credible and "candida" in org: d["cand"] = True
            if credible and "mrsa" in org: d["mrsa"] = True
    rows = []
    for s in sorted(agg):
        d = agg[s]
        rows.append({"strain": s,
                     "anti_Candida": "positive" if d["cand"] else "negative",
                     "anti_MRSA": "positive" if d["mrsa"] else "negative",
