...
The gate_status / warnings / chars are parsed from the real `verify-modeb` output — not
supplied by hand — so the row cannot claim a card passed that did not.
"""
...
import argparse, csv, datetime as dt, re, subprocess, sys
...
def run_gate(card: str, package: str, bgc: str):
    ...
    if runner.is_file():
        cmd = [sys.executable, str(runner), "verify-modeb", card, "--package", package, "--bgc", bgc]
    else:  # defensive fallback if this tool is ever relocated outside a bundle root/tools/ layout
        cmd = [sys.executable, "-m", "mamey", "verify-modeb", card, "--package", package, "--bgc", bgc]
    try:
        out = subprocess.run(cmd, capture_output=True, text=True, timeout=180)
        raw = (out.stdout + out.stderr).strip()
    except Exception as e:  # gate itself failed to run
        return "GATE_ERROR", -1, str(e)
    # verify-modeb prints e.g.:  "... OK — §1–§30 structure + depth (0 warning(s))"
    m = re.search(r"\bOK\b.*?\((\d+)\s+warning", raw)
    if m:
        return "OK", int(m.group(1)), raw
    if re.search(r"\bFAIL\b|missing|residual", raw, re.I):
        return "FAIL", -1, raw
    return "UNKNOWN", -1, raw
