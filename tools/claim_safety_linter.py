...
_DESCRIPTIVE_AFTER_COPULA = {
    ...
    "novel", "rare", "common", "high", "low", "moderate", "strong", "weak",
}
...
_NEGATION_RX = re.compile(r"\b(?:does not|do not|doesn't|don't|cannot|can't|can not|no evidence|"
                          r"not support|not assert|does not assert|not claim|rather than|instead of)\b", re.I)

# CS-P01 / CS-P02 (v9.7.331): a KCB/MIBiG compound name after a copula/production verb is claim-SAFE
# when the SENTENCE frames it as a comparator/anchor/similarity, OR explicitly REJECTS transferring
# it. The fixed ±80/40 char window missed frames placed earlier in the sentence — "the coelibactin
# class anchor is retained", "KCB #1 is the comparator coelibactin", "the aborycin comparator is
# rejected", "similarity, not identity". Check the whole sentence for an explicit hedge/rejection
# token before flagging an identity overclaim. Only explicit tokens suppress — a bare "produces X"
# with no frame still fires.
...
def _sentence_around(text: str, start: int, end: int) -> str:
    """The sentence containing [start, end): bounded by sentence terminators and newlines on both
    sides. Used for the sentence-wide hedge/rejection frame check (CS-P01/CS-P02)."""
    lo = max(text.rfind(". ", 0, start), text.rfind("! ", 0, start),
             text.rfind("? ", 0, start), text.rfind("\n", 0, start))
    lo = lo + 1 if lo >= 0 else 0
    nxts = [p for p in (text.find(". ", end), text.find("! ", end),
                        text.find("? ", end), text.find("\n", end)) if p >= 0]
    hi = min(nxts) + 1 if nxts else len(text)
    return text[lo:hi]
...
def lint_claim_safety(text: str, compound_names: set | None = None) -> list[str]:
    ...
    def _flag(match, is_production: bool = False) -> bool:
        ...
        # CS-P01/CS-P02 (v9.7.331): sentence-wide hedge/rejection frame — a comparator/anchor name or
        # an explicitly rejected name is not an overclaim even when the frame sits before the copula.
        if _IDENTITY_FRAME_RX.search(_sentence_around(text, match.start(), match.end())):
            return False
        if cnames is not None:
            ...
            if any(cn in probe for cn in cnames):
                return True
...
def lint_claim_safety_report(
    ...
    def _identity_hit(m, is_copula: bool) -> bool:
        ...
        if any(frame in context for frame in _CAPACITY_FRAME):
            return False
        # CS-P01/CS-P02 (v9.7.331): sentence-wide hedge/rejection frame exemption (see _IDENTITY_FRAME_RX).
        if _IDENTITY_FRAME_RX.search(_sentence_around(text, m.start(), m.end())):
            return False
        if cnames is not None:
...
def derive_compound_names_from_package(package_dir) -> set[str]:
    ...
    Returns an empty set (not an error) if the triage board is missing, unreadable,
    or yields no resolvable names — callers should treat that as "fall back to the
    heuristic path with a notice", not as a hard failure.
    """
    import csv as _csv
    from pathlib import Path as _Path
