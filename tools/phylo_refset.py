...
def dedup(recs, identity=99.5, min_cov=90.0, report=None, verbose=True):
    ...
    # v9.7.415: the summary line below must say WHICH tiers actually ran. "(K duplicates
    # collapsed)" from Tier 1 alone is indistinguishable from a completed two-tier dedup.
    tier2_status = "Tier 1 only (<2 records; Tier 2 not applicable)"
    if len(tier1) > 1:
        bn = _bin("makeblastdb"); bl = _bin("blastn")
        if not (bn and bl):
            tier2_status = ("TIER 1 ONLY — blast env not found; sequence-level duplicates "
                            "NOT screened")
            if verbose:
            ...
            with tempfile.TemporaryDirectory() as td:
                ...
                with open(fa, "w") as fh:
                    for i, (h, s) in enumerate(tier1):
                        ...
                        idmap[sid] = (h, s)
                        fh.write(f">{sid}\n{s}\n")
                # v9.7.415 — both returncodes are read, matching marker_blast() below. Before
                # this cut neither was: a failed makeblastdb/blastn produced empty stdout, the
                # union-find saw zero pairs, every record became its own cluster, and Tier 2
                # collapsed nothing — while the caller still printed
                # "[dedup] N -> M references (K duplicates collapsed)" from Tier 1 alone. A
                # reference set that was never screened for sequence-level duplicates then went
                # into a tree as if it had been.
                mk = subprocess.run([bn, "-in", fa, "-dbtype", "nucl", "-out", os.path.join(td, "db")],
                                    capture_output=True, text=True)
                if mk.returncode != 0:
                    sys.exit(f"makeblastdb failed (exit {mk.returncode}) building the Tier-2 dedup "
                             f"database; refusing to report a Tier-1-only collapse as a full dedup:"
                             f"\n{mk.stderr[:400]}")
                res = subprocess.run([bl, "-query", fa, "-db", os.path.join(td, "db"),
                                      "-outfmt", "6 qseqid sseqid pident length qlen slen", "-max_target_seqs", "50"],
                                     capture_output=True, text=True)
                if res.returncode != 0:
                    sys.exit(f"blastn failed (exit {res.returncode}) during Tier-2 dedup; refusing "
                             f"to report a Tier-1-only collapse as a full dedup:\n{res.stderr[:400]}")
    ...
    if verbose:
        emit(f"[dedup] {len(recs)} -> {len(kept)} references ({len(collapses)} duplicates "
             f"collapsed; {tier2_status})")
        for c in collapses:
