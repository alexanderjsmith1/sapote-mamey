...
# v9.7.417 (RAZZLE): the 4th arg accepts a STYLE NAME (id | host | noloc | withloc | full) or a bare
# column name, so a figure can be restyled without re-running the placement or the metadata join.
# "withloc"/"noloc" keep their old meaning, so every existing call site is unaffected. An unknown
# style falls back to label_withloc with a warning rather than erroring out mid-figure.
qcol <- switch(locmode,
               "id"      = "label_id",
               "host"    = "label_host",
               "noloc"   = "label_noloc",
               "withloc" = "label_withloc",
               "full"    = "label_full",
               if (locmode %in% names(ann)) locmode else NA_character_)
if (is.na(qcol) || !(qcol %in% names(ann))) {
  warning(sprintf("label style '%s' not available in %s; falling back to label_withloc", locmode, tsv))
  qcol <- "label_withloc"
}
...
# v9.7.417 (RAZZLE): the group name drives the figure TITLE, and it was scraped from the output
# FILENAME -- so naming an output "K_final_wide" titled the figure "K Final Wide". A figure's title
# must come from its data, not from what someone called the file. Order: GG_GROUP env, then the
# `group` column the producer now writes into the annotation TSV, then the old filename scrape.
group <- Sys.getenv("GG_GROUP")
if (!nzchar(group) && "group" %in% names(ann)) {
  g <- unique(ann$group[nzchar(ann$group)]); if (length(g) >= 1) group <- g[1]
}
if (!nzchar(group)) {
...
# v9.7.417 (RAZZLE): GG_WIDTH / GG_HEIGHT override the fixed canvas. Width was hardcoded to 12in
# (14 for >140 tips) regardless of tip count, so a 15-tip tree and a 140-tip tree got the same
# horizontal space and the small one wasted it on whitespace. Widening the canvas stretches the
# x-axis (substitutions/site) WITHOUT changing any branch length -- the scale bar rescales with it,
# so the figure stays quantitatively honest; it is the same tree, drawn wider.
h <- max(4, 0.135 * n + 1.5); w <- if (n > 140) 14 else 12
.envnum <- function(k, d) { v <- Sys.getenv(k); if (nzchar(v)) as.numeric(v) else d }
w <- .envnum("GG_WIDTH",  w)
h <- .envnum("GG_HEIGHT", h)
