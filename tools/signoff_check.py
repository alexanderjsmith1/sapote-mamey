...
def _outgroup_terms(outgroup=None):
    """Normalize --outgroup to a list of lowercase substrings — PARITY with
...
def _is_outgroup_tip(name, outgroup=None):
    """Mirror tree_sanity_check.check(): a tip is an outgroup if its name contains 'OUTGROUP'
    (case-insensitive) OR ANY explicit `outgroup` substring. Keeps this advisory gate in parity
    ...
    nm = (name or "")
    if OUTGROUP.search(nm):
        return True
    return any(term in nm.lower() for term in _outgroup_terms(outgroup))
...
def check_tree_text(nwk, outgroup=None):
    ...
    `outgroup`: optional substring naming an untagged outgroup, matching
...
def main():
    ...
    emit("\n".join(lines))
    emit("  Judgment items to eyeball (not machine-checkable):")
