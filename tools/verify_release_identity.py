...
OPTIONAL_TEXT_PROBES = [
    # Optional accidental-typo rescue shim only. Its content is checked when shipped, but
    # it is not scanner authority and must not be the only ChatGPT target.
...
def _probe_text(name: str, text: str, truth: dict[str, str], where: str) -> list[str]:
    ...
    if name in TEXT_PROBES:
        ...
        if engine and name in {"CHATGPT_START_HERE.md", "CHATGPT_READ_ME_FIRST.md", "CHATGTP_READ_ME_FIRST.md", "CLAUDE_START_HERE.md", "README.md", "TAG", "BUILD_STAMP.txt", "pyproject.toml", "mamey/__init__.py"}:
        ...
        # "PASS" with exit 0.
        if build and name in {"CHATGPT_START_HERE.md", "CHATGPT_READ_ME_FIRST.md", "CHATGTP_READ_ME_FIRST.md", "CLAUDE_START_HERE.md", "README.md", "TAG", "BUILD_STAMP.txt"}:
