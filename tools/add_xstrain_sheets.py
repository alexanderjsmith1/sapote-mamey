...
def main(argv=None) -> int:
    ...
    emit(str(f'added 3 cross-strain sheets | total generated sheets {len(GENERATED_SHEETS)}'), str('class bands:') + " " + str(dict(Counter((band(count) for count in prevalence.values())))), str('cohort context written for') + " " + str(len(context)) + " " + str('strains'), sep="\n")
    if missing:
