...
FILES THIS SCRIPT REWRITES (pure restatements — safe to machine-generate):
  TAG, docs/BUNDLE_CAPABILITIES.md (header line), CITATION.cff, RELEASE_MANIFEST.md
...
RULES = [
    ...
    # AGENTS.md restates bundle/engine/build as a per-release freshness probe that a test
    ...
    ("README.md",
     re.compile(r"Current bundle: sapote-mamey-v\d+(?:\.\d+)*[a-z]* / engine \d+(?:\.\d+)*[a-z]* · build \w+"),
     f"Current bundle: sapote-mamey-v{BUNDLE} / engine {ENGINE} · build {STAMP}"),
    ...
    # v9.7.416: docs/FIGURES_START_HERE.md restates bundle+engine in its own second line and says
    # so ("this line is checked by tools/seal_sweep.py"). The sweep does name it — and has been
    # reporting VERSION_STALE on it — but nothing under sync coverage ever restated it, so the line
    # froze at v9.7.408 / 1.9.148 and shipped that way in .413, .414 and .415 (verified from the
    # three sealed zips). Fourth instance of the exact class already documented above for TAG
    # `build:` (v9.7.153), CITATION `date-released` (v9.7.247) and CURRENT_DOCS_INDEX (v9.7.367):
    # a checked field that no rule owns. --apply now re-stamps it; --check fails on the next freeze.
    ...
    ("RELEASE_MANIFEST.md",
     ...
     f"**Build stamp:** {STAMP}"),
    # The old release manifest carried a historical prose snapshot containing another
    # "authoritative for v..." restatement. That snapshot was retired in v9.7.426; the
    # explicit Authoritative bundle version field above is now the only such anchor.
    ...
    ("bundle_support/project_state_template.json",
    ...
    # GUIDE README packaging stamp; content-review scope is independent of version.
    ("docs/GUIDE/00_README.md",
     re.compile(r"(?:Current as of|Included with) bundle v\d+(?:\.\d+)*[a-z]* / engine Mamey \d+(?:\.\d+)*[a-z]*"),
     f"Included with bundle v{BUNDLE} / engine Mamey {ENGINE}"),
    ...
    # docs/PREREQUISITES.md self-ID in its H1 title (was stale at v9.7.2).
...
def sync_bootstrap_contract(check: bool) -> tuple[bool, list[str]]:
    """v9.7.155: keep the generated bootstrap docs (docs/BOOTSTRAP_FILE_AUDIT.md and the
    ...
    forgetting to RUN it after a bump — which is exactly what left docs/BOOTSTRAP_FILE_AUDIT.md
