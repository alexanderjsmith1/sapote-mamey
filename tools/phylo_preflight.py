...
prevented either a wasted multi-hour run or a wrong figure.
    ...
    R1 registry accession     OUTGROUP_REGISTRY labels GCF_039528075.1 "Actinoplanes missouriensis";
                              it actually downloads as Catellatospora coxensis.

Usage
-----
    phylo_preflight.py <genomes_dir> [--scope FAMILY] [--outgroup SUBSTR] [--min-ratio 1.0] [--json out.json]
...
REGISTRY = f"{ROOT}/OFFICIAL_DATA/OUTGROUP_REGISTRY.tsv"
...
def check_env(rep):
    ...
    if _which("GToTree"):
        try:
            v = subprocess.run(["GToTree", "-v"], capture_output=True, text=True, timeout=30).stdout
            ...
            rep.add("E2", "WARN", "Could not read GToTree version", str(e))
...
def main():
    ...
    # R1 registry sanity (advisory: flags rows whose label/accession pairing is unverified here)
    if os.path.exists(REGISTRY):
        rep.add("R1", "WARN",
                "Verify any registry accession you cite actually returns the labelled organism",
                "known mismatch: GCF_039528075.1 is labelled 'Actinoplanes missouriensis' "
                "but downloads as Catellatospora coxensis")
