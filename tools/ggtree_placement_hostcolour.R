#!/usr/bin/env Rscript
# ggtree_placement_hostcolour.R — variant of ggtree_placement.R that colours the QUERY tip points by
# collection host instead of colouring reference tips by isolation source. Same inputs, same
# annotation TSV; the host is read from the `host` column. Use this when the question is "where do
# the bee-, wasp- and Andrena-derived isolates fall", not "what habitat do the reference species
# come from". Host is DESCRIPTIVE collection metadata, never a tested host association.
...
p <- ggtree(tr, size = 0.32) %<+% ann +
  ...
  geom_tippoint(aes(subset = (kind == "query" & !is.na(host) & host != ""), color = host),
                size = 2.4, na.rm = TRUE) +
  scale_color_manual(name = "Collection host",
    values = c("Bombus sp."="#8856a7","Apis mellifera"="#d94801","Apidae"="#e08214",
               "Wasp"="#1b7837","Andrena sp."="#4aa3c7"), na.translate = FALSE) +
