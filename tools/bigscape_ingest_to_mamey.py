...
from mamey.bigscape_namespace import (
    ...
    public_error, strain_from_gbk_name, validate_membership_rows,
)
...
def parse_locator(path):
    ...
    # Use the shared resolver so underscore-joined organism names remain intact.
    strain = strain_from_gbk_name(b)
    m = LOCATOR.search(b)
    if m:
