...
def find_workspace_root(package: str | Path, max_up: int = 12) -> Path:
    ...
    names). Precedence:
      ...
      2. nearest ancestor of the package containing a generic project marker (.claude/.git/…).
      3. the current working directory (where the operator ran mamey).
      4. a bounded ancestor of the package.
    """
...
def _strain_dirs_in(root: Path, strain: str) -> list:
    ...
    try:
        if any(p.is_dir() and p.name.upper().startswith("BGC") for p in root.iterdir()):
            dirs.append(root)
    except OSError:
        pass
    # de-dup
            ...
            for sdir in _strain_dirs_in(Path(root), strain):
                for bgc_dir in sorted(p for p in sdir.iterdir() if p.is_dir()
                    ...
                    for g in globs:
                        ...
                        hits = [h for h in bgc_dir.glob(g) if _has_data_row(h)]
                        if hits:
...
def ingested(package: str | Path) -> dict:
    ...
    if store.is_dir():
        for f in store.glob("*_online_blastp.csv"):
