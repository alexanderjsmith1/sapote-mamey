...
import json, glob, os, csv, ast, collections, re, argparse, hashlib
import json as _j
import contextlib
import math
...
def _save_pair(fig, path, *, renderer="cohort_figures", provenance="normalized_cohort_tables"):
    ...
    if _engine is not None:
        try:
            ...
            pass  # best-effort: never let a layout-engine API change break a figure save
    target = Path(path)
    save_figure(
...
def _load_manifest_short(pkg, sid):
    ...
    if os.path.exists(ms_path):
        try:
            return _loadj(ms_path)
        except (json.JSONDecodeError, OSError):
            pass  # corrupt/partial write — fall through to reconstruction
    ...
    if os.path.exists(man_path):
        try:
            ...
            # writer (models.py::MameyRun.to_manifest_dict) only ever puts it at
            # man["bgc_counts"]["assembly_tier"] — the v9.7.160 comment named the wrong nested
            # key, so read_manifest_field()'s own _ASSEMBLY_NESTED table pointed at a location
            # that has never held this field, and this line silently reconstructed None on every
            # single-strain gold run (manifest_short.json doesn't exist yet at this call site).
            # Fixed at the schema layer (manifest_schema.py `_BGC_COUNTS_NESTED` now carries
            # F_ASSEMBLY_TIER / F_INTERIOR_PCT) — this call now resolves correctly.
            ...
            # corrected_bgcs was never reconstructed here at all (bgc_counts["corrected"]) —
            # every consumer of ms["corrected_bgcs"]/read_manifest_field("corrected_bgcs",
            # manifest_short=ms) fell through to its own `default` (usually 0) on every
            # single-strain gold run, before manifest_short.json exists on disk. Confirmed live
            # on a real AS-XXX run: D05 (scatter_raw_vs_corrected) plotted the strain's point at
            # y=0 instead of the real corrected_bgcs=24.75.
            ms["corrected_bgcs"] = read_manifest_field("corrected_bgcs", manifest=man)
        except (json.JSONDecodeError, OSError):
            pass
    # Last-resort defaults so downstream int()/sort never crashes.
    ms.setdefault("raw_bgcs", 0)
    ms.setdefault("assembly_tier", "UNKNOWN")
    ms.setdefault("corrected_bgcs", 0)
    return ms
...
def _ripp_product_count(pkg):
    ...
    if not inv:
        return 0
    try:
        return sum(1 for row in csv.DictReader(open(inv, encoding="utf-8"))
...
def figs_multi(S, order, OUT, *, f13_cohort_manifest=None,
    ...
    for s in order:
        for da in S[s]["gene"].get("domain_arch",[]):
            try: a=ast.literal_eval(da["architecture"]).get("domain_counts",{})
            except Exception: a={}
...
def generate(runs_dir="runs_gold", out="cohort_figures", strains=None, public_only=False, series="F",
    ...
    try:
        write_captions(out, [os.path.basename(p) for p in pngs])  # defined below (merged from cohort_figure_captions)
    except Exception:
        pass
...
def build(S, order, OUT):
    ...
    for s in order:
        for r in inv[s]:
            bid=r.get("BGC_ID")
            try: L[(s,bid)]=float(r.get("Length_kb") or 0)
            except Exception: L[(s,bid)]=0
...
def _strain_profiles(S, order):
    ...
    for s in order:
        ...
        for da in S[s]["gene"].get("domain_arch",[]):
            try: a=_ast.literal_eval(da["architecture"]).get("domain_counts",{})
            except Exception: continue
...
def _strain_summary_row(strain_id, pkg_dir, result):
    ...
    interior, edge, fc = _boundary_counts(pkg_dir)
...
def build_cohort_figures(results, outdir, logger=None):
    ...
    try:
        ...
        log(f"COHORT_FIGS_SKIPPED: {type(e).__name__}: {e}")
        return {"status": "SKIPPED_ERROR", "reason": f"{type(e).__name__}: {e}", "figure_count": 0}
