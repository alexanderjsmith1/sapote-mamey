...
`docs/Sapote_Mamey_ROADMAP.md` documents under "The order operations SHOULD run":
...
def stage_strain_dir(input_zip: str | Path, dest: str | Path) -> Path:
    ...
    if not region_gbks:
        raise ValueError(
            ...
            f"found after extraction. See docs/COMPANION_FILES.md."
        )
        ...
        try:
            ...
            report["steps"]["bgc_walk"] = {"status": "SKIPPED",
                                            "skipped_reason": f"{type(e).__name__}: {e} "
                                                               ...
                                                               "bundle_support/install_sapote_addons.sh)"}
    else:
        report["steps"]["bgc_walk"] = {"status": "SKIPPED",
