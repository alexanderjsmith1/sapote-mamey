...
def _scan_lookup(run: Any) -> tuple[dict[str, tuple[str, str]], str | None]:
    """Return recorded scan states and a typed read failure, if one occurred."""
    ...
    try:
        ...
        return {}, f'INVALID_SCAN_STATUS: {type(exc).__name__}'
    return scans, None
...
def build_output_checklist(run: Any, package_dir: str | Path) -> list[dict[str, str]]:
    ...
    raw_n = len(bgcs)
    scan, scan_diagnostic = _scan_lookup(run)
    ...
    if scan_diagnostic:
        rows.append({
            'Phase': 'Source scans',
            'Output_or_task': 'Recorded source-scan status',
            'Expected_artifact_or_evidence': f'{strain}_3_scan_states.json / manifest.json source_scans',
            'Status': scan_diagnostic,
            'Progress': 'unreadable',
            'Evidence': 'runtime scan_status',
            'Next_action': 'Repair the recorded scan-status structure before Sapote judgment.',
        })
    for name, label in [
...
def write_output_checklist(run: Any, package_dir: str | Path) -> tuple[Path, Path]:
    ...
    except (OSError, UnicodeDecodeError, _json.JSONDecodeError, TypeError, AttributeError) as exc:
        rows.append({
            'Phase': 'Judgment',
            'Output_or_task': 'Compiled manifest for Sapote Mode B status',
            'Expected_artifact_or_evidence': 'manifest.json',
            'Status': f'BLOCKED_INVALID_MANIFEST: {type(exc).__name__}',
            'Progress': 'unreadable',
            'Evidence': 'manifest.json',
            'Next_action': 'Repair the manifest before treating Mode B progress as complete.',
        })
