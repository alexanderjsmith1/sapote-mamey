...
def _coordinates_for_json_row(
    ...
    raw_end = row.get("end")
    issues: list[str] = []
    json_coordinates: tuple[int, int] | None = None
    ...
    if raw_start is not None and raw_end is not None:
        try:
            start = json_integer(raw_start)
            end = json_integer(raw_end)
            if start < 0 or end <= start:
                raise ValueError("invalid coordinate interval")
            json_coordinates = (start + 1, end)
        except (TypeError, ValueError, OverflowError):
            issues.append("INVALID_JSON_COORDINATES")
    elif raw_start is not None or raw_end is not None:
        issues.append("INVALID_JSON_COORDINATES")

    raw_strand = row.get("strand")
    strand = 0
    if raw_strand not in (None, ""):
        try:
            strand = json_integer(raw_strand)
            if strand not in (-1, 0, 1):
                raise ValueError("invalid strand")
        except (TypeError, ValueError, OverflowError):
            strand = 0
            issues.append("INVALID_JSON_STRAND")

    def source_with_issues(source: str) -> str:
        return source if not issues else f"{source}_AFTER_{'_AND_'.join(issues)}"

    if json_coordinates is not None:
        return (
            contig, json_coordinates[0], json_coordinates[1], strand,
            source_with_issues("JSON_COORDINATES"),
        )
    ...
    if loc_start is not None and loc_end is not None:
        return contig, loc_start, loc_end, loc_strand, source_with_issues("JSON_LOCATION")
    cds = _resolve_locus(
        contig,
    ...
    if cds is not None:
        return (
            ...
            source_with_issues("CDS_LOCUS_COORDINATES"),
        )
    return contig, None, None, 0, source_with_issues("NO_COORDINATE")
