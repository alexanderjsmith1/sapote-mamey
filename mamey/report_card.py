...
def _kcb_named(bgc: dict) -> tuple[str, float]:
    raw = (bgc.get("KCB_top") or "").strip()
    name = raw.split("|")[1].strip() if "|" in raw else ("" if raw.upper() in ("", "NONE") else raw)
    try:
        score = float(bgc.get("KCB_score") or 0)
    except Exception:
        score = 0.0
    return name, score


def _num(bgc: dict, key: str) -> float:
    try:
        return float(bgc.get(key) or 0)
    except Exception:
        return 0.0
...
def _badges(bgc: dict, poly: dict | None) -> dict:
    ...
    novelty = {"LOW": "LOW (known — strong named-compound KCB)",
               ...
               "MODERATE": "MODERATE (known family)"}[tier]
...
def build_card(bgc: dict, poly: dict | None, modules: list[dict], strain: str, version: str,
               contig_region_count: int, rescues: list | None = None) -> str:
    loc = _locator(bgc)
    b = _badges(bgc, poly)
    name, score = _kcb_named(bgc)
    ...
    kcb_line = f"KCB `{name}` (score {score:.0f}) — **similarity, not identity**" if name else "KCB: none (KCB-dark)"
...
def render_report_cards(package_dir: str, bgc_id: str | None = None, out: str | None = None) -> dict:
    ...
    if os.path.exists(manp):
        try:
            wv = _loadj(manp).get("workflow_version")
            if wv:
                version = wv
        except Exception:
            pass
    ...
    try:
        ...
        _resc = lambda bid: _cv.rescue_for_bgc(package_dir, bid)
    except Exception:
        _resc = lambda bid: []
    cards = []
    for b in rows:
        ...
        cards.append(build_card(b, pr, _modules_for(b), S, version,
                                ...
                                rescues=_resc(b.get("BGC_ID") or b.get("bgc_id") or "")))
