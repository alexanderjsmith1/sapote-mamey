...
class BlastpScopeError(ValueError):
    """A requested exact BGC/region scope could not be resolved safely."""


class BlastpCrosswalkError(BlastpScopeError):
    """An existing crosswalk cannot be discovered or read safely."""


class BlastpKcbAnchorError(ValueError):
    """An existing KCB/MIBiG profile cannot support a trustworthy anchor."""
...
def _matching_files(directory: Path, pattern: str, error_type, code: str) -> list[Path]:
    """Match one directory level while distinguishing absence from traversal failure."""
    try:
        ...
        raise error_type(
            f"{code}: cannot traverse {directory} ({type(exc).__name__})"
        ) from exc
...
def _find_crosswalk(args) -> dict | None:
    ...
    Returns None when no crosswalk exists. Existing unreadable or malformed evidence raises a
    typed hold so the caller can refuse with the cause intact."""
    import csv as _csv
    ...
    def _load(csv_path) -> dict | None:
        try:
            with open(csv_path, encoding="utf-8-sig") as _fh:  # BLAST-P07: with-block (was a bare open handle leak)
                ...
                if not reader.fieldnames or "bgc_id" not in reader.fieldnames:
                    raise BlastpCrosswalkError(
                        f"BLASTP_CROSSWALK_HOLD: {Path(csv_path).name!r} lacks bgc_id"
                    )
                rows = list(reader)
        except BlastpCrosswalkError:
            raise
        except (OSError, UnicodeError, _csv.Error) as exc:
            raise BlastpCrosswalkError(
                f"BLASTP_CROSSWALK_HOLD: cannot read {Path(csv_path).name!r} "
                f"({type(exc).__name__})"
            ) from exc
        out = {}
        for row in rows:
            bgc_id = str(row.get("bgc_id", "")).strip()
            if bgc_id in out and row != out[bgc_id]:
                raise BlastpCrosswalkError(
                    f"BLASTP_CROSSWALK_HOLD: conflicting duplicate {bgc_id!r}"
                )
            if bgc_id:
                out[bgc_id] = row
        return out
    ...
    def _search(d: Path) -> dict | None:
        for cand in (d, d.parent, d / "package", d.parent / "package"):
            hits = _matching_files(
                cand, "*_2b_bgc_crosswalk.csv", BlastpCrosswalkError,
                "BLASTP_CROSSWALK_HOLD",
            )
            if len(hits) > 1:
                raise BlastpCrosswalkError(
                    f"BLASTP_CROSSWALK_HOLD: multiple crosswalks under {cand}"
                )
            if hits:
                return _load(hits[0])
        return None
    ...
    if xw:
        xp = Path(xw)
        try:
            mode = xp.stat().st_mode
        except FileNotFoundError as exc:
            raise BlastpCrosswalkError(
                f"BLASTP_CROSSWALK_HOLD: explicit crosswalk path does not exist: {xp}"
            ) from exc
        except OSError as exc:
            raise BlastpCrosswalkError(
                f"BLASTP_CROSSWALK_HOLD: cannot inspect explicit crosswalk {xp} "
                f"({type(exc).__name__})"
            ) from exc
        ...
        if _stat.S_ISDIR(mode):
            ...
            raise BlastpCrosswalkError(
                f"BLASTP_CROSSWALK_HOLD: explicit crosswalk path is not a file or directory: {xp}"
            )
...
def _derive_kcb_anchor(args) -> tuple[str, int | None]:
    ...
    Missing profile evidence remains UNDETERMINED. Existing unreadable or malformed profile
    evidence raises a typed hold."""
    ...
    for root in roots:
        hits = _matching_files(
            root, "*_3_mibig_profile.csv", BlastpKcbAnchorError,
            "BLASTP_KCB_ANCHOR_HOLD",
        )
        if len(hits) > 1:
            raise BlastpKcbAnchorError(
                f"BLASTP_KCB_ANCHOR_HOLD: multiple MIBiG profiles under {root}"
            )
        if hits:
            prof = hits[0]
            break
    ...
    try:
        with open(prof, encoding="utf-8-sig") as fh:
            ...
            if not reader.fieldnames or "bgc_id" not in reader.fieldnames:
                raise BlastpKcbAnchorError(
                    f"BLASTP_KCB_ANCHOR_HOLD: {prof.name!r} lacks bgc_id"
                )
            for r in reader:
                ...
                if cov is None:
                    raw_cov = r.get("dominant_distinct_query_genes")
                    if str(raw_cov or "").strip():
                        try:
                            cov = int(float(raw_cov)) or None
                        except (TypeError, ValueError) as exc:
                            raise BlastpKcbAnchorError(
                                f"BLASTP_KCB_ANCHOR_HOLD: invalid coverage for {bgc!r}"
                            ) from exc
                break
    except BlastpKcbAnchorError:
        raise
    except (OSError, UnicodeError, _csv.Error) as exc:
        raise BlastpKcbAnchorError(
            f"BLASTP_KCB_ANCHOR_HOLD: cannot read {prof.name!r} "
            f"({type(exc).__name__})"
        ) from exc
    return kcb_top, cov
...
def _scope_feats(feats, bgc=None, region=None, crosswalk=None):
    ...
    if bgc and crosswalk and bgc in crosswalk:
        ...
        try:
            ...
            raise BlastpScopeError(
                f"BLASTP_SCOPE_HOLD: {bgc} has an invalid crosswalk coordinate window"
            ) from exc
        if win[0] < 0 or win[1] <= win[0]:
            raise BlastpScopeError(
                f"BLASTP_SCOPE_HOLD: {bgc} has a non-positive or reversed crosswalk window"
            )
    elif region:
...
def blastp_online_command(args) -> int:
    ...
    if _bgc or _region:
        ...
        except (BlastpScopeError, BlastpKcbAnchorError) as exc:
            _scope_hold = str(exc)
            _scoped = []
        if _scoped:
            feats = _scoped
        elif _bgc or _region:
            emit(f"[blastp-online] REFUSING: {_scope_hold + '; ' if _scope_hold else ''}"
                  "requested BGC/region locator did not resolve to any "
                  "CDS features. Pass the matching sealed package crosswalk and antiSMASH input; "
                  "an unresolved locator is never widened to an unscoped submission.")
            return 1
    ...
    with open(out_csv, "w", newline="", encoding="utf-8") as fh:
        ...
        node_region = ""
        if _xwalk and _bgc_id and _bgc_id in _xwalk:
            _r = _xwalk[_bgc_id]
