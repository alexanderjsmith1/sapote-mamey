...
import sys
import warnings
from .exact_identity import ExactLocusIdentityError, exact_locus_from_native_inventory_row
...
def inspect_command(args) -> int:
    ...
    if gbk_regions:
        try:
            ...
            warnings.warn(
                f"ANTISMASH_VERSION_PROBE_UNAVAILABLE: {exc}",
                RuntimeWarning,
                stacklevel=2,
            )
    ...
    if not as_version:
        try:
            ...
            warnings.warn(
                f"ANTISMASH_VERSION_FALLBACK_UNAVAILABLE: {exc}",
                RuntimeWarning,
                stacklevel=2,
            )
...
def _cohort_locator(r) -> str:
    ...
    if reg:
        raw = reg[6:] if reg.lower().startswith("region") else reg
        try:
            number = float(raw)
            if not number.is_integer() or number <= 0:
                raise ValueError
            reg = region_label(int(number))
        except (TypeError, ValueError, OverflowError) as exc:
            raise ExactLocusIdentityError(f"invalid cohort region {reg!r}") from exc
...
def _bgc_json_row(r, strain) -> dict:
    ...
    """
    identity = exact_locus_from_native_inventory_row(strain, r)
    return {
        "strain": identity.strain,
        "region": identity.region,
        "exact_locus": identity.exact_locus,
        "bgc_id":      r.get("BGC_ID", ""),
        "contig":      r.get("Contig", ""),
...
def _read_bgc_board(board_path):
    """Validate native identities before filtering; never hide a malformed row."""
    board_path = Path(board_path)
    suffix = "_4_triage_board.csv"
    if not board_path.name.endswith(suffix):
        raise ExactLocusIdentityError("expected a strain-bound triage-board filename")
    strain = board_path.name[:-len(suffix)]
    if not strain:
        raise ExactLocusIdentityError("missing strain in triage-board filename")
    with board_path.open(newline="", encoding="utf-8") as handle:
        reader = csv.DictReader(handle)
        headers = reader.fieldnames or []
        required = {"Strain", "BGC_ID", "Contig", "Node_ID", "antiSMASH_Region"}
        if not required.issubset(headers) or len(headers) != len(set(headers)):
            raise ExactLocusIdentityError("missing or duplicate native identity columns")
        rows = list(reader)
    aliases = set()
    for ordinal, row in enumerate(rows, 1):
        if None in row or any(value is None for value in row.values()):
            raise ExactLocusIdentityError(f"malformed CSV row {ordinal}")
        if not row["Strain"].strip():
            raise ExactLocusIdentityError(f"missing native strain at row {ordinal}")
        try:
            identity = exact_locus_from_native_inventory_row(strain, row)
        except ExactLocusIdentityError as exc:
            raise ExactLocusIdentityError(f"triage row {ordinal}: {exc}") from exc
        if identity.bgc_alias in aliases:
            raise ExactLocusIdentityError(f"duplicate BGC alias at triage row {ordinal}")
        aliases.add(identity.bgc_alias)
    return strain, rows


def _select_bgc_rows(rows, axis="rank", top_n=None, include_dropped=False):
...
def bgc_json_list_from_board(board_path, axis="rank", top_n=None, include_dropped=False) -> list:
    """Build identity-validated CLI JSON records for CLI and cohort consumers.

    The board filename binds the strain; any supplied row strain must agree.
    Missing/conflicting identity raises ExactLocusIdentityError before any rows
    are returned, including when a filter would otherwise hide the invalid row.
    """
    strain, rows = _read_bgc_board(board_path)
    rows = _select_bgc_rows(rows, axis, top_n, include_dropped)
    return [_bgc_json_row(r, strain) for r in rows]
...
def list_bgcs_command(args) -> int:
    ...
    if len(boards) != 1:
        emit("ERROR: expected exactly one *_4_triage_board.csv in package", file=sys.stderr)
        return 1
    axis = getattr(args, "axis", "rank")
    try:
        out = bgc_json_list_from_board(
            boards[0], axis, getattr(args, "top_n", None),
            getattr(args, "include_dropped", False),
        )
    except (ExactLocusIdentityError, OSError, csv.Error) as exc:
        emit(f"IDENTITY_HOLD: {exc}", file=sys.stderr)
        return 1
