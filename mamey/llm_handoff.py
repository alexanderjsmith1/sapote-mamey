...
START_FILE_NAMES = ["AGENTS.md", "CLAUDE.md", "PATCH_CHAT_START_HERE.md"]
...
def build_handoff_receipt(
    ...
    *,
    ...
    start_here_found = any(
        path.name in START_FILE_NAMES or bool(re.search(r"START_HERE|READ_ME_FIRST", path.name, re.I))
        for path in [bundle_root / f.path for f in files]
    )
    has_shared_contract = any(Path(f.path).name in {"AGENTS.md", "CLAUDE.md"} for f in files)
    has_chatgpt = has_shared_contract or any(f.llm_hint == "chatgpt" for f in files)
    has_claude = has_shared_contract or any(f.llm_hint == "claude" for f in files)
