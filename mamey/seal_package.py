...
def _triage_rows(package_dir: Path) -> tuple[dict[str, dict], str | None]:
    """Map BGC_ID to triage rows and distinguish absence from load failure."""
    rows = {}
...
def _read_filed_card(package_dir: Path, bgc_id: str) -> tuple[str | None, str | None]:
    """Read a register-declared COMPLETE card with typed failure detail.

    Optional absence is decided before this helper: callers invoke it only for
    IDs returned by ``list_complete_bgcs``.  At that point a missing, empty, or
    unreadable canonical file is contradictory persisted state, not "no card".
    """
    from mamey.judgment_store import _mode_b_path

    path = _mode_b_path(package_dir, bgc_id)
    if not path.is_file():
        return None, f"filed card missing at canonical path {path.name}"
    try:
        card = path.read_text(encoding="utf-8")
    except (OSError, UnicodeDecodeError) as exc:
        return None, f"filed card unreadable ({type(exc).__name__}: {exc})"
    if not card.strip():
        return None, f"filed card empty at canonical path {path.name}"
    return card, None
...
def _gate_mode_b_quality(package_dir: Path, triage: dict) -> GateResult:
    try:
        from mamey.judgment_store import list_complete_bgcs, read_register
    except Exception as e:
        return GateResult("mode_b_quality", "FAIL", True,
                          detail=f"judgment-store import unavailable: {e}")
    register_status = str(read_register(package_dir).get("judgment_status") or "")
    if register_status.startswith("CORRUPT"):
        return GateResult(
            "mode_b_quality",
            "FAIL",
            True,
            detail=(f"judgment register is {register_status}; filed-card quality "
    ...
    try:
        ...
        return GateResult("mode_b_quality", "FAIL", True,
                          detail=f"quality-gate import unavailable: {e}")
    findings = []
    for bid in bgcs:
        card, card_error = _read_filed_card(package_dir, bid)
        if card_error:
            findings.append({"bgc": bid, "severity": "WARN", "detail": card_error})
            continue
        ...
        try:
            ...
            findings.append({
                "bgc": bid,
                "severity": "WARN",
                "detail": f"quality evaluation error: {type(exc).__name__}: {exc}",
            })
            continue
...
def _gate_locator(
    package_dir: Path,
    triage: dict,
    triage_load_error: str | None = None,
) -> GateResult:
    try:
        from mamey.judgment_store import list_complete_bgcs
    except Exception as e:
        return GateResult("locator_reconciliation", "FAIL", True,
                          detail=f"judgment-store import unavailable: {e}")
    if triage_load_error:
        return GateResult(
            "locator_reconciliation",
            "FAIL",
            True,
            detail=("triage board present but unreadable; locator cannot be "
                    f"reconciled ({triage_load_error})"),
        )
    bgcs = list_complete_bgcs(package_dir)
    if not bgcs:
    ...
    try:
        ...
        return GateResult("locator_reconciliation", "FAIL", True,
                          detail=f"locator-gate import unavailable: {e}")
    if not triage:
    ...
    for bid in bgcs:
        ...
        if card_error:
            blocking_hit = True
            findings.append({"bgc": bid, "severity": "FAIL", "detail": card_error})
            continue
        ...
        if not row:
            ...
            findings.append({
                "bgc": bid,
                "severity": "FAIL",
                "detail": "filed card has no matching triage row; locator cannot be reconciled",
            })
            continue
        assert card is not None
        ...
        try:
            ...
            findings.append({
                "bgc": bid,
                "severity": "FAIL",
                "detail": f"locator evaluation error: {type(exc).__name__}: {exc}",
            })
            continue
...
def _gate_claim_safety(package_dir: Path) -> GateResult:
    try:
        from mamey.judgment_store import list_complete_bgcs, _strain_compound_names
    except Exception as e:
        return GateResult("claim_safety", "FAIL", True,
                          detail=f"judgment-store import unavailable: {e}")
    bgcs = list_complete_bgcs(package_dir)
    if not bgcs:
    ...
    try:
        ...
        return GateResult("claim_safety", "FAIL", True,
                          detail=f"claim-safety import unavailable: {e}")
    compound_load_errors: list[dict[str, str]] = []
    cnames = _strain_compound_names(package_dir, compound_load_errors) or None
    findings = [{
        "bgc": "",
        "severity": "FAIL",
        "detail": ("triage board unreadable while loading compound names "
                   f"({row['error_type']}: {row['message']})"),
    } for row in compound_load_errors]
    identity_hit = bool(compound_load_errors)
    for bid in bgcs:
...
def seal_package(package_dir: str | Path, strict: bool = False, advisory: bool = False) -> dict:
    ...
    triage, triage_load_error = _triage_rows(pkg)

    gates = [
        _gate_package_validate(pkg),
        _gate_workbook(pkg),
        _gate_mode_b_quality(pkg, triage),
        _gate_locator(pkg, triage, triage_load_error),
