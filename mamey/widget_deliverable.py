...
def _num(value: Any, default: float | None = 0.0) -> float | None:
    ...
    try:
        ...
        raise ValueError("WIDGET_NUMBER_INVALID: expected a finite number") from exc
    if not math.isfinite(number):
        raise ValueError("WIDGET_NUMBER_INVALID: expected a finite number")
    return number


def _percentage(value: Any) -> float | None:
    """Preserve unreported percentages; reject malformed present measurements."""
    if value is None or (isinstance(value, str) and not value.strip()):
        return None
    try:
        number = float(value)
    except (TypeError, ValueError) as exc:
        raise ValueError("WIDGET_PERCENTAGE_INVALID: expected a finite number") from exc
    if not math.isfinite(number):
        raise ValueError("WIDGET_PERCENTAGE_INVALID: expected a finite number")
    return number
...
def _int(value: Any, default: int | None = 0) -> int | None:
    ...
    try:
        ...
        raise ValueError("WIDGET_INTEGER_INVALID: expected a finite integer") from exc
    if not math.isfinite(number) or not number.is_integer():
        raise ValueError("WIDGET_INTEGER_INVALID: expected a finite integer")
    return int(number)
...
class PackageSource:
    ...
    def _current_snapshot(self) -> dict[str, str]:
        """Read current bytes, including replacement ZIPs and added directory files."""
        paths = [self.path] if self.kind == "zip" else sorted(
        ...
        for path in paths:
            ...
            with path.open("rb") as handle:
                for chunk in iter(lambda: handle.read(1024 * 1024), b""):
                    digest.update(chunk)
            key = path.name if self.kind == "zip" else path.relative_to(self._root).as_posix()
    ...
    def verify_unchanged(self) -> None:
        try:
            ...
            raise ValueError("WIDGET_SOURCE_CHANGED: source is no longer readable") from exc
        if current != self._initial_snapshot:
            raise ValueError("WIDGET_SOURCE_CHANGED: package bytes changed during rendering")
    ...
    def locate(self, suffix: str, *, required: bool = False) -> str | None:
        ...
        if len(matches) > 1:
            raise ValueError(f"WIDGET_SOURCE_AMBIGUOUS: multiple artifacts match {suffix}")
        if matches:
            return matches[0]
        if required:
    ...
    def json(self, suffix: str, *, required: bool = False) -> tuple[dict, str | None]:
        ...
        if not name:
            return {}, None
        try:
            data = json.loads(self.read_bytes(name).decode("utf-8-sig"))
        except (UnicodeDecodeError, json.JSONDecodeError) as exc:
            raise ValueError(f"WIDGET_JSON_INVALID: {name}: {exc}") from exc
        if not isinstance(data, dict):
            raise ValueError(f"WIDGET_JSON_OBJECT_REQUIRED: {name}")
        return data, name

    def _csv_rows(self, name: str) -> list[dict[str, str]]:
        try:
            text = self.read_bytes(name).decode("utf-8-sig")
            reader = csv.reader(io.StringIO(text), strict=True)
            header = next(reader, None)
            if header is None:
                return []
            if not all(header) or len(set(header)) != len(header):
                raise ValueError(f"WIDGET_CSV_HEADER_INVALID: {name}")
            rows = []
            for row in reader:
                if not row:
                    continue
                if len(row) != len(header):
                    raise ValueError(f"WIDGET_CSV_WIDTH_INVALID: {name}: line {reader.line_num}")
                rows.append(dict(zip(header, row)))
            return rows
        except (UnicodeDecodeError, csv.Error) as exc:
            raise ValueError(f"WIDGET_CSV_INVALID: {name}: {exc}") from exc

    def csv(self, suffix: str) -> tuple[list[dict[str, str]], str | None]:
        name = self.locate(suffix)
        if not name:
            return [], None
        return self._csv_rows(name), name
    ...
    def csv_many(self, directory_name: str) -> list[tuple[list[dict[str, str]], str]]:
        ...
        for name in sorted(names):
            out.append((self._csv_rows(name), name))
        return out
...
def _join_rows(triage: list[dict], inventory: list[dict]) -> list[dict]:
    ...
    for pos, row in enumerate(source, 1):
        ...
        out.append({
            ...
            "kcb_score": _num(merged.get("KCB_score"), None),
            "kcb_proteins": _int(merged.get("KCB_proteins"), None),
...
def _top_blastp_hit(row: dict[str, str]) -> dict[str, Any]:
    return {
        ...
        "pct_identity": _percentage(row.get("pct_identity")),
        "pct_positives": _percentage(row.get("pct_positives")),
        "query_coverage": _percentage(row.get("query_coverage")),
    ...
    *,
    ...
    for row in mibig:
        ...
        mibig_by_gene.setdefault(key, []).append({
            ...
            "pct_identity": _percentage(row.get("pct_identity")),
            "pct_coverage": _percentage(row.get("pct_coverage_interpretation") or row.get("pct_coverage")),
            "evalue": _text(row.get("evalue")),
            "coverage_qc_flag": _text(row.get("coverage_qc_flag")),
        })
    for hits in mibig_by_gene.values():
        hits.sort(key=lambda row: (row["reference_rank"], -(row["pct_identity"] if row["pct_identity"] is not None else -1), row["mibig_accession"]))
    ...
    for row in clusterblast:
        ...
        cluster_by_gene.setdefault(key, []).append({
            ...
            "subject_gene": _text(row.get("subject_gene")),
            "pct_identity": _percentage(row.get("pct_identity")),
            "pct_coverage": _percentage(row.get("pct_coverage")),
            "evalue": _text(row.get("evalue")),
        })
    for hits in cluster_by_gene.values():
        hits.sort(key=lambda row: (row["reference_rank"], -(row["pct_identity"] if row["pct_identity"] is not None else -1), row["reference"]))
...
def _identity_region(value: Any) -> str:
    match = re.fullmatch(r"(?:region)?0*(\d+)", str(value).strip(), re.I)
    if not match or int(match[1]) < 1:
        raise ValueError("WIDGET_IDENTITY_MISSING: valid region required")
    return f"region{int(match[1]):03d}"


def _identity_map(strain: str, inventory: list[dict], groups: list[list[dict]], pairs: list[dict]) -> dict:
    identities = {}
    for row in inventory:
        alias = _text(row.get("BGC_ID") or row.get("bgc_id"))
        contig = _text(row.get("Contig"))
        if not alias or not contig or not strain or strain == "UNKNOWN":
            raise ValueError("WIDGET_IDENTITY_MISSING: strain, full contig and alias required")
        if alias in identities:
            raise ValueError("WIDGET_IDENTITY_DUPLICATE: inventory alias is not unique")
        region = _identity_region(row.get("Region") or row.get("antiSMASH_Region"))
        identities[alias] = dict(strain=strain, contig=contig, region=region,
                                bgc_id=alias, full_identity=f"{strain} / {contig} / {region} / {alias}")

    def check(row: dict, alias: str, suffix: str = "") -> None:
        identity = identities.get(alias)
        if identity is None:
            raise ValueError("WIDGET_IDENTITY_UNBOUND: row absent from inventory")
        for field, keys in [("strain", ("strain", "Strain")),
                            ("contig", ("contig" + suffix, "Contig" + suffix)),
                            ("region", ("region" + suffix, "Region" + suffix, "antiSMASH_Region" + suffix))]:
            for key in keys:
                value = _text(row.get(key))
                if not value:
                    continue
                actual = _identity_region(value) if field == "region" else value
                expected = identity[field]
                if (actual.upper() if field == "strain" else actual) != (expected.upper() if field == "strain" else expected):
                    raise ValueError(f"WIDGET_IDENTITY_CONFLICT: {field} disagrees with inventory")
    for row in inventory:
        check(row, _text(row.get("BGC_ID") or row.get("bgc_id")))
    for group in groups:
        for row in group:
            check(row, _text(row.get("BGC_ID") or row.get("bgc_id")))
    for row in pairs:
        for side in ("a", "b"):
            check(row, _text(row.get("bgc_" + side) or row.get("BGC_" + side.upper())), "_" + side)
    return identities


def _domain_coverage(inventory: list[dict], domains: list[dict], sidecar: str | None) -> dict:
    inventory_ids = [_text(row.get("BGC_ID") or row.get("bgc_id")) for row in inventory]
    domain_ids = [_text(row.get("bgc_id") or row.get("BGC_ID")) for row in domains]
    known = set(inventory_ids) - {""}
    reported = set(domain_ids) - {""}
    represented = sum(bool(key) and key in reported for key in inventory_ids)
    duplicates = len(domain_ids) - len(set(domain_ids))
    unmapped = sum(not key or key not in known for key in domain_ids)
    inconsistent = duplicates or unmapped or "" in inventory_ids or len(known) != len(inventory_ids)
    status = ("NOT_PROVIDED" if sidecar is None else
              "INVENTORY_UNAVAILABLE" if not inventory else
              "INCONSISTENT" if inconsistent else
              "COMPLETE" if represented == len(inventory) else "PARTIAL")
    return {"status": status, "inventory_rows": len(inventory),
            "represented_inventory_rows": represented,
            "not_represented_rows": len(inventory) - represented,
            "sidecar_rows": len(domains), "duplicate_rows": duplicates,
...
def _load_model(source: PackageSource) -> dict:
    ...
    identities = _identity_map(strain, inventory, [triage, domains, genes], rggmci)
    ...
    model = {
        ...
        "domain_coverage": _domain_coverage(inventory, domains, domains_name),
    ...
    for key in ("priority", "domains", "genes"):
        for row in model[key]:
            row.update(identities[row["bgc_id"]])
    for row in model["rggmci"]:
        for side in ("a", "b"):
            identity = identities[row["bgc_" + side]]
            row.update({key + "_" + side: value for key, value in identity.items()})
        row["full_pair"] = row["full_identity_a"] + " ↔ " + row["full_identity_b"]
    for row in model["gene_roster"].get("bgcs", []):
        if row.get("bgc_id") not in identities:
            raise ValueError("WIDGET_IDENTITY_UNBOUND: roster absent from inventory")
        identity = identities[row["bgc_id"]]
        if row.get("node") and row["node"] != identity["contig"]:
            raise ValueError("WIDGET_IDENTITY_CONFLICT: roster contig disagrees with inventory")
        row.update(identity)
    return model
...
function exportSvg(id,name,identity){const node=document.getElementById(id);if(!node)return;const x=node.cloneNode(true);x.setAttribute('xmlns','http://www.w3.org/2000/svg');if(identity){const context=document.createElementNS('http://www.w3.org/2000/svg','desc');context.textContent=identity;x.insertBefore(context,x.firstChild)}saveBlob(new Blob([new XMLSerializer().serializeToString(x)],{type:'image/svg+xml'}),name)}
...
(()=>{const all=D.priority;let shown=[];function draw(){const k=kind.value,qq=q.value.toLowerCase();shown=all.filter(r=>{const ok=k==='all'||(k==='cctt'&&r.cctt)||(k==='kcb'&&r.kcb_top)||(k==='guard'&&(r.standing_rule||r.misanchor))||(k==='dark'&&!r.cctt&&!r.kcb_top);return ok&&(!qq||[r.full_identity,r.products,r.cctt,r.kcb_top,r.standing_rule,r.misanchor].join(' ').toLowerCase().includes(qq))});const vals=[['Rows',shown.length],['CCTT present',shown.filter(r=>r.cctt).length],['KCB present',shown.filter(r=>r.kcb_top).length],['Guarded',shown.filter(r=>r.standing_rule||r.misanchor).length]];kpis.innerHTML=vals.map(x=>`<div class="card"><div class="eyebrow">${x[0]}</div><div class="kpi">${x[1].toLocaleString()}</div></div>`).join('');rows.innerHTML=shown.map(r=>`<tr><td><strong>${esc(r.full_identity)}</strong></td><td>${esc(r.products)}</td><td>${esc(r.cctt)||'<span class="muted">not recorded</span>'}</td><td>${esc(r.kcb_top)||'<span class="muted">not recorded</span>'}</td><td>${r.kcb_score??'not reported'}</td><td>${esc(r.standing_rule)}</td><td>${esc(r.misanchor)}</td><td>${esc(r.safe_claim)}</td></tr>`).join('')}kind.onchange=draw;q.oninput=draw;csv.onclick=()=>exportCsv(shown,D.meta.strain_id+'_evidence_filtered.csv');draw()})();
"""+'</script>'
...
const hasIdentity=h=>h&&h.pct_identity!==null&&h.pct_identity!==undefined&&Number.isFinite(Number(h.pct_identity))&&Number(h.pct_identity)>=0;
...
for(const v of uniq(all.map(r=>r.bgc_id)))geneBgc.insertAdjacentHTML('beforeend',`<option value="${esc(v)}">${esc(all.find(r=>r.bgc_id===v).full_identity)}</option>`);
...
const similarityChannels=[
 ...
 {key:'mibig',shape:'diamond',jitter:3,hit:g=>g.mibig_hits.filter(hasIdentity).reduce((a,h)=>!a||h.pct_identity>a.pct_identity?h:a,null),label:'MIBiG'},
 {key:'clusterblast',shape:'triangle',jitter:8,hit:g=>g.clusterblast_hits.filter(hasIdentity).reduce((a,h)=>!a||h.pct_identity>a.pct_identity?h:a,null),label:'ClusterBlast'}
];
...
geneCsv.onclick=()=>exportCsv(shown.map(r=>({full_identity:r.full_identity,strain:r.strain,bgc_id:r.bgc_id,contig:r.contig,region:r.region,region_locator:r.region_locator,locus_tag:r.locus_tag,aa_length:r.aa_length,bgc_start:r.bgc_start,bgc_end:r.bgc_end,cds_start:r.cds_start,cds_end:r.cds_end,strand:r.strand,products:r.products,boundary:r.boundary,domains:r.domains,reaction_family:r.reaction_family,reaction_hint:r.reaction_hint,mibig_compounds:r.mibig_hits.map(h=>h.mibig_compound).join('; '),clusterblast_references:r.clusterblast_hits.map(h=>h.reference_source).join('; '),blastp_channels:Object.keys(r.blastp).join('; ')})),D.meta.strain_id+'_gene_evidence_filtered.csv');
geneMapSvg.onclick=()=>exportSvg('geneMap',(selected?selected.full_identity.replace(/[^A-Za-z0-9._-]+/g,'_'):'no_locus_selected')+'_gene_map.svg',selected&&selected.full_identity);
geneSimilaritySvg.onclick=()=>exportSvg('geneSimilarity',(selected?selected.full_identity.replace(/[^A-Za-z0-9._-]+/g,'_'):'no_locus_selected')+'_gene_similarity.svg',selected&&selected.full_identity);
...
def _safe_gene_roster(source, strain_id: str | None = None) -> dict:
    ...
    try:
        if getattr(source, "kind", None) == "zip":
            import tempfile
            import stat
            manifest_name = source.locate("manifest.json", required=True)
            prefix = PurePosixPath(manifest_name).parent
            # The legacy reader needs filesystem CSV/JSON inputs. Materialize only
            # those types in a disposable directory after validating member paths.
            selected = []
            for name in source._names:
                member = PurePosixPath(name)
                if member.is_absolute() or ".." in member.parts or "\\" in name:
                    raise ValueError("unsafe roster archive member")
                info = source._zip.getinfo(name)
                if stat.S_ISLNK(info.external_attr >> 16):
                    raise ValueError("roster archive symlinks are not supported")
                if member.is_relative_to(prefix) and member.suffix.lower() in {".csv", ".json"}:
                    selected.append((name, member.relative_to(prefix)))
            if len({str(rel) for _, rel in selected}) != len(selected):
                raise ValueError("duplicate roster archive members")
            with tempfile.TemporaryDirectory(prefix="mamey-roster-") as tmp:
                root = Path(tmp)
        ...
        if getattr(source, "kind", None) != "directory":
            raise ValueError("unsupported roster source type")
        ...
        raise ValueError("WIDGET_GENE_ROSTER_INVALID: package-native roster could not be read") from exc
...
const guardOf=Object.fromEntries((D.priority||[]).map(r=>[r.bgc_id,r.standing_rule||r.misanchor||'']));
const bsel=$('gbgc');R.bgcs.forEach((b,i)=>bsel.insertAdjacentHTML('beforeend',`<option value="${i}">${esc(b.full_identity)} · ${esc((b.products||'').split(';')[0])}${guardOf[b.bgc_id]?' [EXCLUDED]':''}</option>`));
...
function draw(){const b=R.bgcs[st.bgc],G=b.genes,gd=guardOf[b.bgc_id]||'',gdBadge=gd?`<span class="badge bad" title="${esc(gd)}">EXCLUDED FROM LEAD RANKING</span> `:'';const f=$('gfocus');f.innerHTML=`<option value="all">All ${G.length} genes</option>`+G.map(g=>`<option${g.locus_tag==st.focus?' selected':''}>${esc(g.locus_tag)}</option>`).join('');
 ...
 $('gcard').innerHTML=gdBadge+(g?`<strong>${esc(g.locus_tag)}</strong> · ${g.aa||'?'} aa · ${g.strand||'?'} · `+CH.map(([k,l])=>{const c=g.channels[k];return `${l} <strong>${c&&c.pid!=null?c.pid+'%':'—'}</strong>`;}).join(' · ')+` · role: ${esc(g.role)} · domains: ${esc((g.domains||[]).join(', ')||g.smcog||'—')}`
   :`<strong>${esc(b.full_identity)}</strong> · ${esc(b.node)} · ${esc(b.products)} · ${esc(b.boundary||'')} · ${b.length_kb||'?'} kb · KCB: ${esc(kcb(b.kcb_top))} — hover a gene`);
 // arrow map
...
function exportName(suffix){return R.bgcs[st.bgc].full_identity.replace(/[^A-Za-z0-9._-]+/g,'_')+suffix}
$('gmapexport').onclick=()=>exportSvg('gmap',exportName('_gene_roster_map.svg'),R.bgcs[st.bgc].full_identity);
$('gscatexport').onclick=()=>exportSvg('gscat',exportName('_gene_roster_similarity.svg'),R.bgcs[st.bgc].full_identity);
$('gcsvexport').onclick=()=>{const b=R.bgcs[st.bgc];const rows=b.genes.filter(g=>st.focus==='all'||g.locus_tag===st.focus).map(g=>({full_identity:b.full_identity,strain:b.strain,contig:b.contig,region:b.region,bgc_id:b.bgc_id,locus_tag:g.locus_tag,aa_length:g.aa,start:g.start,end:g.end,strand:g.strand,role:g.role,domains:(g.domains||[]).join('; '),...Object.fromEntries(CH.map(([c])=>[c+'_identity',cv(g,c)]))}));exportCsv(rows,exportName('_gene_roster.csv'))};
draw();})();
"""
...
def _gene_page(data: dict) -> str:
    ...
    body = (
        ...
        '<div class="controls"><button id="gmapexport">Export map SVG</button>'
        '<button id="gscatexport">Export similarity SVG</button>'
        '<button id="gcsvexport">Export selected genes CSV</button></div>'
...
def render_widget_deliverable(package: str | Path, outdir: str | Path | None = None) -> dict:
    ...
    try:
        ...
        # Invalidate old completion receipts before replacing any output. An
        # interrupted refresh may leave partial pages, but cannot remain sealed.
        markers = (out / "WIDGET_MANIFEST.json", out / "SHA256SUMS.txt")
