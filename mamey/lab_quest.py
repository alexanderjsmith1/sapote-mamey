...
def load_package_snapshot(package: str | Path) -> PackageSnapshot:
    ...
    for row in triage_rows:
        ...
        exact = row_identity(row)
        if exact != identity_by_alias[alias]:
            raise ValueError("triage identity conflicts with inventory identity")
        triage.append({**row, "exact_locus": exact})
