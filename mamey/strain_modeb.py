...
try:
    from .exact_identity import ExactLocusIdentityError, exact_locus_from_mapping, exact_locus_from_native_inventory_row, exact_locus_from_native_manifest_bgc
except ImportError:  # standalone `python strain_modeb.py`
    ...
    from mamey.exact_identity import ExactLocusIdentityError, exact_locus_from_mapping, exact_locus_from_native_inventory_row, exact_locus_from_native_manifest_bgc  # type: ignore
...
def _identity_index(strain: str, tri) -> dict[str, str]:
    ...
    for row in tri.to_dict(orient="records"):
        ...
        if "Contig" in row and "Node_ID" in row:
            identities[alias] = exact_locus_from_native_inventory_row(strain, row).exact_locus
        else:
...
def build_strain_citation_receipt(md: str, pkg_dir: str | Path) -> dict:
    ...
    for row in rows:
        ...
        try:
            if "contig" in row:
                full_contigs[alias] = exact_locus_from_native_manifest_bgc(strain, row).full_contig
            else:
                exact_locus_from_mapping(strain, row)
                full_contigs[alias] = _clean(row.get("node_id"))
        except ExactLocusIdentityError as exc:
            raise StrainCitationReceiptError("STRAIN_CITATION_CROSSWALK_IDENTITY_INVALID") from exc
        by_alias[alias] = row
    ...
    for index, (tag, _label) in enumerate(REQUIRED_STRAIN_SECTIONS):
        ...
        for match in exact_matches:
            ...
            if row is None or full_contigs[alias] != node or _clean(row.get("antismash_region")).casefold() != region.casefold():
        ...
        known_nodes = set(full_contigs.values())
