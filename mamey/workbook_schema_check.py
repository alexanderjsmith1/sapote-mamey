        ...
        if hdr[:len(D5_COLUMNS)] != D5_COLUMNS:
            ...
            for rn, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
                ...
                try:
                    ...
                    errs.append(
                        f"Fragment_Rescue_Tiers row {rn}: cannot validate tier inputs "
                        f"({type(exc).__name__})")
            ...
            if ws.max_row is None or ws.max_column is None:
                try:
                    ...
                    results["column_errors"].append({
                        "sheet": sheet_code,
                        "code": "DIMENSION_SCAN_FAILED",
                        "error": f"Cannot determine worksheet dimensions ({type(exc).__name__})",
                    })
                    results["status"] = "FAIL"
                    return results  # The enclosing finally still closes the workbook.
