...
def render_all(package_dir: str | Path,
    ...
    try:
        manifest_payload = json.loads((pkg / "manifest.json").read_text(encoding="utf-8"))
    except (OSError, UnicodeError, json.JSONDecodeError) as exc:
        return {
            "package": str(pkg),
            "ok": False,
            "error": f"manifest.json unreadable: {type(exc).__name__}: {exc}",
            "sets": {},
        }
    if not isinstance(manifest_payload, dict):
        return {
            "package": str(pkg),
            "ok": False,
            "error": "manifest.json invalid: top-level JSON object required",
            "sets": {},
        }
...
def _resolve_source_zip(pkg: Path, explicit: str | None = None) -> str | None:
    ...
    basename beside the package tree. Any resolved ZIP must match the manifest's
    exact `input_zip_sha256`; malformed/unreadable authority surfaces. Returns None
    only when no source locator is declared or its bytes are no longer available,
    which routes domain-level into its explicitly receipted LIMITED mode."""
    import json
    ...
    if not man.exists():
        if explicit:
            ...
            if not candidate.is_file():
                raise FileNotFoundError(f"EXPLICIT_SOURCE_ZIP_UNAVAILABLE: {candidate.name}")
            raise ValueError("SOURCE_ZIP_DIGEST_UNBOUND: manifest.json is unavailable")
        return None
    ...
    if explicit:
        ...
        candidate = original if original.is_file() else pkg.parent / original.name
        if not candidate.is_file():
            return None
    else:
        return None
    ...
    if not isinstance(expected, str) or not re.fullmatch(r"[0-9a-f]{64}", expected):
        raise ValueError("SOURCE_ZIP_DIGEST_UNBOUND: manifest input_zip_sha256 is unavailable or invalid")
    ...
    if observed != expected:
        raise ValueError(
            "SOURCE_ZIP_DIGEST_MISMATCH: resolved source ZIP does not match manifest input_zip_sha256"
        )
...
def _run_domain_level(pkg: Path, top_n: int = 10, **_kw) -> dict[str, Any]:
    ...
    if not (dl_dir / "domain_complexity_metrics_by_bgc.csv").exists():
        # Try to compute it. The post-seal producer is optional, but a failed
        # attempt is not the same condition as absent domain-level evidence.
        try:
            from .domain_level import run_domain_level
            attempt = run_domain_level(
                pkg,
                source_antismash=_resolve_source_zip(pkg, _kw.get("source_antismash")),
                top_n=top_n,
            )
        except Exception as exc:
            return {"status": "ERRORED",
                    "error": f"domain-level producer failed: {type(exc).__name__}: {exc}"}
    if not (dl_dir / "domain_complexity_metrics_by_bgc.csv").exists():
        if isinstance(attempt, dict) and attempt.get("reason"):
            reason = str(attempt["reason"])
        else:
            reason = "no domain_level CSV data on disk"
        return {"status": "SKIPPED",
...
def figure_stack_preflight() -> tuple[bool, str]:
    ...
    try:
        ...
        return False, ("Figure stack MISSING \u2014 matplotlib not installed. Most figure sets will "
                       ...
                       "(bash bundle_support/install_sapote_addons.sh) or `pip install matplotlib`, then re-run.")
    ...
    for d in sorted(src_dirs, key=str):
        ...
        for f in sorted(d.glob("*.png")) + sorted(d.glob("*.svg")):
            ...
            try:
                ...
                # Gathering is nonblocking, but a produced figure must not disappear
                # from the handoff without a machine-readable diagnostic.
