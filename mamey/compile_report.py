...
def _read_manifests(pkg: Path) -> dict[str, Any]:
    ...
    if (pkg / "manifest_short.json").exists():
        try: ms = json.loads((pkg / "manifest_short.json").read_text(encoding="utf-8"))
        except Exception: ms = {}
    if (pkg / "manifest.json").exists():
        try: man = json.loads((pkg / "manifest.json").read_text(encoding="utf-8"))
...
def _decision_for(row: dict) -> tuple[str, str]:
    ...
    Vocabulary per §13.12: PRIORITY ISO / HIGH SEQ / MEDIUM ACT / LOW / EXCL."""
    ...
    corrected_rank_blank = corrected_rank_col_present and not (row.get("Corrected_rank") or "").strip()
    ...
    ab, af = _f("AB_auto"), _f("AF_auto")
    score = max(ab, af)
    # rank-aware top leads get isolation priority
    try: crank = int(row.get("Corrected_rank") or row.get("Rank") or 999)
    except (TypeError, ValueError): crank = 999
...
def _blastp_from_channel_stores(pkg: Path) -> str:
    ...
    for sdir, lab in present:
        for f in sorted(sdir.glob("*_top10.csv")):
            ...
            try:
                rows = [r for r in csv.DictReader(f.open(newline="", encoding="utf-8")) if str(r.get("hit_rank", "")).strip() == "1"]
...
def _figures(pkg: Path, generate: bool = True) -> str:
...
def _fermentation_draft(pkg: Path) -> str:
    ...
    if "siderophore" in joined or "metallophore" in joined:
        class_notes.append("Siderophore/metallophore capacity → run an iron-limited condition to "
                           "de-repress; CAS assay for siderophore activity (ecological, not a kill claim).")
    return "\n".join([
...
def _key_findings(pkg: Path, m: dict) -> str:
    ...
    nnov = sum(1 for r in rows if _score(r) == 0)
    if nnov:
...
def build_report(package_dir: str | Path, generate_figures: bool = True,
    ...
    parts = [
        ...
        _assembly_table(m),
        _triage_board(pkg),
        _figures(pkg, generate=generate_figures),
