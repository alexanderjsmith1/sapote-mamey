...
def _bgc_context_from_package(package: str | None, bgc: str | None, _cross: bool = True) -> dict | None:
    ...
    try:
        ...
        for _cds in list(pkg.glob("*_cds_table.csv")) + list(pkg.glob("cds_table.csv")):
            with open(_cds, encoding="utf-8", errors="ignore") as _fh:
    ...
    try:
        ...
        if gt2:
            ...
            with open(gt2, encoding="utf-8", errors="ignore") as _fh:
    ...
    try:
        ...
        if _man is not None:
            import csv as _csv3
            with open(_man, encoding="utf-8", errors="ignore") as _fh:
    ...
    try:
        import csv
        gt = next(pkg.glob("*_gene_by_gene_all_bgcs.csv"), None)
        if gt:
            n_core = 0
            with open(gt, encoding="utf-8") as _gt_fh:  # P3a: with-block + encoding (was a bare open handle leak)
                _gt_rows = list(csv.DictReader(_gt_fh))
            for row in _gt_rows:
    ...
    try:
        ...
        tctx = {}
    if not any(k in ctx for k in ("kcb_top", "KCB_top")):
        ...
        if gc:
            for line in gc.read_text(encoding="utf-8").splitlines():
                ...
                if str(r.get("bgc_id") or "").strip() == str(bgc).strip():
                    ...
                    ctx["tta_by_gene"] = {g["locus_tag"].lower(): g.get("tta_codons", 0)
...
def verify_modeb_command(args) -> int:
    ...
    findings = lint_card(md, bgc_context=ctx, check_depth=True, strict_depth=strict, check_class_content=True, check_claim_safety=True, check_evidence_presence=True,
