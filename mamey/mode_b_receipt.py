...
def _context_failure(code: str, source: Path, role: str) -> dict[str, Any]:
    """Path-safe ERROR for a present context source that cannot be trusted."""
    return {
        "severity": "ERROR",
        "code": code,
        "section": None,
        "message": (
            ...
            "verification context. Repair or replace the package artifact before ingest."
        ),
    }


def _record_context_failure(ctx: dict[str, Any], code: str,
                            source: Path, role: str) -> None:
    finding = _context_failure(code, source, role)
    findings = ctx.setdefault(_CONTEXT_FINDINGS_KEY, [])
    if finding not in findings:
        findings.append(finding)


def _context_findings(ctx: dict[str, Any] | None) -> list[dict[str, Any]]:
    return list((ctx or {}).get(_CONTEXT_FINDINGS_KEY, []))


def _exact_identity_or_unbound(pkg: Path, strain: str, bgc_id: str) -> str:
    """Return the complete package-bound locus identity, or typed missingness."""
    try:
        from .exact_identity import exact_locus_display
        return exact_locus_display(
            strain,
            _package_node_for_bgc(pkg, bgc_id),
            _package_region_for_bgc(pkg, bgc_id),
            bgc_id,
        )
...
def _extend_persistence_warnings(
    ...
    """Propagate producer warnings with an exact identity and deterministic dedup."""
    for raw in (record_result or {}).get("persistence_warnings", []) or []:
        if not isinstance(raw, dict):
            continue
        warning = {
            "identity": _exact_identity_or_unbound(pkg, strain, bgc_id),
            "stage": str(raw.get("stage") or "unknown"),
            "error_type": str(raw.get("error_type") or "UnknownError"),
...
def _persistence_advisory(summary: dict[str, Any]) -> str:
    """Render one typed advisory for nonblocking persistence sidecar failures."""
    warnings = summary.get("persistence_warnings", []) or []
    if not warnings:
        return ""
    details = "; ".join(
        f"{row.get('identity', 'UNBOUND')} [{row.get('stage', 'unknown')}] "
        f"{row.get('error_type', 'UnknownError')}: {row.get('message', '')}"
        for row in warnings
    )
    return f"WARNING: PERSISTENCE_ADVISORY ({len(warnings)}): {details}"
    ...
    *,
    ...
    Returns a summary dict::
        ...
        {"strain_id", "recorded": [bgc_ids],
         ...
         "persistence_warnings": [{identity, stage, error_type, message}],
    ...
    from .judgment_store import (
        judgment_status_blocks_ingest, read_register, record_mode_b,
    )
    try:
    ...
    if judgment_status_blocks_ingest(reg.get("judgment_status")):
        raise ValueError(
    ...
    for card in receipt["cards"]:
        ...
        if True:  # v9.7.409 A7 (G3): the gate block now ALWAYS runs; an unavailable gate raises into the
            ...
            try:
                ...
                findings = _context_findings(ctx) + _lint_card(
                    mode_b_md, bgc_context=ctx, check_evidence_presence=True
                )  # item 4: §4 coverage/named-subject enforced at receipt
        ...
        _extend_persistence_warnings(
            persistence_warnings, record_result, pkg, package_strain, bgc_id
        )
        if n_errors:
...
def auto_detect_ingest(package_dir: str | Path,
    ...
    for card_path in cards:
        ...
        try:
            ...
            summary["skipped_unreadable"].append({
                "identity": _exact_identity_or_unbound(pkg, strain, bgc_id),
                "error_type": type(_read_exc).__name__,
            })
            continue
        if not content.strip():
            summary["skipped_no_content"].append(
                _exact_identity_or_unbound(pkg, strain, bgc_id)
            )
            continue
        ...
        if True:  # v9.7.409 A7 (G3): same fail-closed routing as ingest_receipt above.
            try:
                ...
                ctx = _bgc_context_from_triage(pkg, bgc_id)
                findings = _context_findings(ctx) + _lint_card(
                    content, bgc_context=ctx, check_evidence_presence=True
                )  # item 4: §4 coverage enforced at receipt
                n_errors = sum(1 for f in findings
                               if f.get("severity") == "ERROR")
            except Exception as _gate_exc:
        ...
        try:
            record_result = record_mode_b(
                pkg, bgc_id, mode_b_md=content, session_id=session_id,
                rank=_q_rank, edge_status=_q_edge, cds_count=_q_cds,
            )
            _extend_persistence_warnings(
                summary["persistence_warnings"], record_result, pkg, strain, bgc_id
            )
            if n_errors:
            ...
            summary["record_failures"].append({
                "identity": _exact_identity_or_unbound(pkg, strain, bgc_id),
                "error_type": type(_record_exc).__name__,
            })
            continue
...
def ingest_one_card(package_dir: str | Path,
    ...
    """
    from .judgment_store import (
        _strain_from_pkg, judgment_status_blocks_ingest, read_register, record_mode_b,
    )
    ...
    reg = read_register(pkg)
    if judgment_status_blocks_ingest(reg.get("judgment_status")):
        summary["status"] = "REGISTER_NOT_INGESTIBLE"
        summary["judgment_status"] = reg.get("judgment_status")
        _not_evaluated(
            "REGISTER_NOT_INGESTIBLE",
            f"judgment register status is {reg.get('judgment_status')}",
        )
        return summary
    known_bgcs = reg.get("bgcs", {})
    if bgc_id not in known_bgcs:
    ...
    try:
        ...
        structure_findings = _context_findings(bgc_ctx) + lint_card(
            content, bgc_context=bgc_ctx, check_evidence_presence=True
        )  # item 4: §4 coverage enforced at receipt
    except Exception as _gate_exc:
...
def _bgc_context_from_triage(pkg: Path, bgc_id: str, _cross: bool = True) -> dict:
    ...
    by reading the relevant triage row. An absent optional board returns {};
    a present unreadable or malformed board returns a structured ERROR finding.
    ...
    try:
        with open(candidates[0], newline="", encoding="utf-8") as f:
            ...
            if not fields.intersection({"BGC_ID", "bgc_id"}):
                raise ValueError("BGC identity column absent")
            rows = list(reader)
            if any(None in row or any(value is None for value in row.values()) for row in rows):
                raise ValueError("row width differs from header width")
    except OSError:
        return {_CONTEXT_FINDINGS_KEY: [
            _context_failure("VERIFICATION_CONTEXT_UNREADABLE", candidates[0], "triage board")
        ]}
    except (UnicodeError, _csv.Error, ValueError):
        return {_CONTEXT_FINDINGS_KEY: [
            _context_failure("VERIFICATION_CONTEXT_MALFORMED", candidates[0], "triage board")
        ]}
    ...
    def _read(glob_pat, role):
        ...
        try:
            ...
            _record_context_failure(
                ctx, "VERIFICATION_CONTEXT_UNREADABLE", cands[0], role
            )
            return []
        except (_csv.Error, ValueError):
            _record_context_failure(
                ctx, "VERIFICATION_CONTEXT_MALFORMED", cands[0], role
            )
            return []

    # module_count: aSModule features ONLY — counting aSDomain rows would fire
    # §32-§34 on domains without a programmed assembly line.
    n_mod = sum(1 for r in _read("*_3_antismash_modules.csv", "antiSMASH modules table")
    ...
    n_prot = sum(1 for r in _read("*_2b_bgc_crosswalk.csv", "BGC crosswalk")
    ...
    n_4a = 0
    for r in _read("*_4A_RGGMCI_ranked_pairs.csv", "RG-GMCI ranked-pairs table"):
    ...
    n_4d = 0
    for r in _read("*_4D_two_proof_rescue.csv", "two-proof rescue table"):
    ...
    if _cross:
        try:
            ...
            for k, v in pctx.items():
                if k == _CONTEXT_FINDINGS_KEY:
                    findings = ctx.setdefault(_CONTEXT_FINDINGS_KEY, [])
                    for finding in list(v or []):
                        if finding not in findings:
                            findings.append(finding)
                else:
                    ctx[k] = v
            ...
            _record_context_failure(
                ctx, "VERIFICATION_CONTEXT_MERGE_FAILED",
                pkg / "manifest.json", "package context merge"
            )
    ...
    if "cds_count" not in ctx:
        for _gc_path in sorted(pkg.glob("*gene_context.jsonl")):
            try:
                ...
                _record_context_failure(
                    ctx, "VERIFICATION_CONTEXT_UNREADABLE", _gc_path,
                    "gene-context ledger"
                )
            except (json.JSONDecodeError, AttributeError):
                _record_context_failure(
                    ctx, "VERIFICATION_CONTEXT_MALFORMED", _gc_path,
                    "gene-context ledger"
                )
            if "cds_count" in ctx:
                break
...
def lint_card_in_package(package_dir: str | Path, bgc_id: str,
    ...
    - Never raises: gate/context failures become explicit ERROR findings rather
      than a result indistinguishable from a clean lint.
    """
    pkg = Path(package_dir)
    try:
        from .modeb_structure_gate import lint_card as _lint_card
        ...
        return [{
            "severity": "ERROR", "code": "GATE_UNAVAILABLE", "section": None,
            "message": (
                "Mode B structure gate is unavailable "
                f"({type(_import_exc).__name__}); card is UNVERIFIED, not clean."
            ),
        }]
    ...
    try:
        ...
        ctx = {}
        context_errors = [{
            "severity": "ERROR", "code": "VERIFICATION_CONTEXT_UNAVAILABLE",
            "section": None,
            "message": (
                "Package verification context could not be built "
                f"({type(_ctx_exc).__name__}); card is UNVERIFIED, not clean."
            ),
        }]
    else:
        context_errors = _context_findings(ctx)
    try:
        return context_errors + _lint_card(card_md, bgc_context=ctx)
    except Exception as _context_lint_exc:
        # last-resort: context-free lint so the author still gets structure info
        context_lint_failure = {
            "severity": "ERROR", "code": "CONTEXT_LINT_FAILED",
            "section": None,
            "message": (
                "Context-aware lint failed "
                f"({type(_context_lint_exc).__name__}); context-free findings follow."
            ),
        }
        try:
            return context_errors + [context_lint_failure] + _lint_card(card_md)
        except Exception as _bare_lint_exc:
            return context_errors + [context_lint_failure, {
                "severity": "ERROR", "code": "GATE_UNAVAILABLE",
                "section": None,
                "message": (
                    "Mode B structure gate failed in context-aware and context-free modes "
                    f"({type(_bare_lint_exc).__name__}); card is UNVERIFIED, not clean."
                ),
            }]
...
def ingest_receipts_command(args) -> int:
    ...
    if card:
        ...
        _advisory = _persistence_advisory(card_summary)
        emit(
            f"  register status:   {card_summary['judgment_status'] or '(unknown)'}",
    ...
    if auto:
        ...
        if summary.get("skipped_misnamed"):
            recovery_diagnostics.extend([
                f"  WARNING (misnamed): {', '.join((f'{bid} ({fn})' for bid, fn in summary['skipped_misnamed']))}",
                f"    -> cards must be named '{summary['strain_id']}_<BGC>_mode_b.md'; rename and re-run, or these will not be ingested.",
            ])
        if summary.get("skipped_unreadable"):
            recovery_diagnostics.append(
                f"  FAILED (unreadable): {len(summary['skipped_unreadable'])} card(s); "
                "repair the judgment files and re-run auto-detect"
            )
        if summary.get("skipped_no_content"):
            recovery_diagnostics.append(
                f"  SKIPPED (empty): {len(summary['skipped_no_content'])} card(s); "
                "empty files are not completed Mode B cards"
            )
        if summary.get("record_failures"):
            recovery_diagnostics.append(
                f"  FAILED (persistence): {len(summary['record_failures'])} card(s); "
                "the durable judgment register was not updated"
            )
        if recovery_diagnostics:
            emit(*recovery_diagnostics, sep="\n", file=sys.stderr)
        _advisory = _persistence_advisory(summary)
        emit(
            f"  register status:   {summary['judgment_status'] or '(unknown)'}",
            *([_advisory] if _advisory else []),
            sep="\n",
        )
        # Exit codes: 0 normal; 3 if any cards were rejected for structure
        # (so a CI run can detect that and fail)
        from .judgment_store import judgment_status_blocks_ingest
        if judgment_status_blocks_ingest(summary.get("judgment_status")):
            return 1
        if summary.get("skipped_unreadable") or summary.get("skipped_no_content") or summary.get("record_failures"):
            return 1
    ...
    if summary["e1_updated"]:
        ...
        _workbook_status = (
            f"  workbook E1:       updated "
            f"({es.get('bgcs_complete', '?')} complete / "
            f"{es.get('bgcs_written', '?')} rows)"
        )
    else:
