...
def classify_reference_dark(
    ...
    """
    qc = _to_int(query_gene_count)
    rc = _to_int(recognizable_gene_count)
    frac = _to_float(recognizable_gene_fraction)
    denom = f"({rc}/{qc} genes)"
...
def main(argv=None) -> int:
    ...
    counts = summarize(rows)
    emit(f"reference-dark prior for {args.package_dir}: {counts}  (n={len(rows)})")
    if out:
        emit(f"wrote {out}")
    return 0
