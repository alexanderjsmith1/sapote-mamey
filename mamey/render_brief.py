...
def load_facts(pkg):
    ...
    norm = []
    for r in rows:
        ab, ab_state = _num_state(col(r, "AB_auto"))
        af, af_state = _num_state(col(r, "AF_auto"))
        novelty, novelty_state = _num_state(col(r, "Novelty_auto"))
        norm.append({
            ...
            "af": af,
            "novelty": novelty,
            "ab_input_state": ab_state,
            "af_input_state": af_state,
...
def _num(x):
    """Backward-compatible numeric value for legacy callers; use `_num_state` for provenance."""
    return _num_state(x)[0]
...
def fig_landscape(rows, png_path, strain, plt):
    ...
    for y, r in zip(ys, ordered):
        ...
        if r.get("ab_input_state", "MEASURED") == "MEASURED":
            ax.barh(y + 0.18, r["ab"], height=0.34, color=c, zorder=3)
        else:
            ax.scatter(1, y + 0.18, marker="x", s=22, color=CL["exc"], zorder=4)
        if r.get("af_input_state", "MEASURED") == "MEASURED":
    ...
    unresolved = sum(1 for r in ordered for key in ("ab_input_state", "af_input_state")
                     if r.get(key, "MEASURED") != "MEASURED")
    if unresolved:
        foot += f" \\u00b7 {unresolved} unresolved AB/AF value(s) marked ×, not plotted as zero"
    if n_more > 0:
    ...
    with open(_data_csv_tmp, "w", newline="") as f:
        ...
        w.writerow(["rank", "bgc_id", "contig", "region", "boundary", "arch",
                    "ab_auto", "ab_input_state", "af_auto", "af_input_state",
                    "novelty_auto", "novelty_input_state", "lead_tier_auto", "kcb_score", "cctt_triggers",
        ...
        for r in rows:
            ...
            w.writerow([r["rank"], r["bgc_id"], r["contig"], r["region"], r["boundary"], r["arch"],
                        r["ab"], r.get("ab_input_state", "MEASURED"),
                        r["af"], r.get("af_input_state", "MEASURED"),
                        r["novelty"], r.get("novelty_input_state", "MEASURED"),
                        r["lead_tier"], r["kcb_score"], r["cctt"],
                        shown, sacc])
...
def render_brief(pkg, tier="standard", logger=None):
    ...
    try:
        ...
        numeric_issues = []
        strain_id = str(facts.get("manifest", {}).get("strain_id") or "UNBOUND")
        for row in facts["rows"]:
            fields = [field for field in ("ab", "af", "novelty")
                      if row.get(f"{field}_input_state", "MEASURED") != "MEASURED"]
            if fields:
                identity = (f"{strain_id} / {row.get('contig') or 'UNBOUND'} / "
                            f"{row.get('region') or 'UNBOUND'} / {row.get('bgc_id') or 'UNBOUND'}")
                states = ", ".join(
                    f"{field}={row.get(f'{field}_input_state', 'UNBOUND')}" for field in fields
                )
                log(f"BRIEF_NUMERIC_INPUT_UNRESOLVED: {identity}: {states}")
                numeric_issues.append((row, fields))
        abaf_unresolved = any("ab" in fields or "af" in fields for _, fields in numeric_issues)
        ...
        if abaf_unresolved:
            log("SAPOTE_FIGS_SKIPPED: BRIEF_NUMERIC_INPUT_UNRESOLVED: "
                "AB_auto/AF_auto inputs must be measured before score-dependent figures render")
        else:
            try:
        ...
        try:
            ...
            if _os.path.exists(_pairs_csv):
                ...
                _rescue_fig = fig_rggmci_rescue(
                    _pairs_csv, _rescue_png, facts["strain_label"], plt, logger=log
                )
        ...
        _ferm_text = ""
        _reg = {}
        _judgment_register_corrupt = False
        try:
            ...
            _reg_status = str(_reg.get("judgment_status") or "")
            if _reg_status.startswith("CORRUPT"):
                _judgment_register_corrupt = True
                _error_types = sorted({
                    str(item.get("error_type") or "UNKNOWN")
                    for item in (_reg.get("load_errors") or []) if isinstance(item, dict)
                })
                _error_note = ",".join(_error_types) if _error_types else "UNSPECIFIED"
                log(f"JUDGMENT_REGISTER_CORRUPT: {_reg_status}: load_errors={_error_note}; "
                    "completion is unresolved, not absent or pending")
        except Exception as e:
            ...
            if tier == "standard":
                ...
                for _p in produced:
                    ...
                    try:
                        ...
                        log(f"BRIEF_IMAGE_SKIPPED: {os.path.basename(_p)}: {type(exc).__name__}: {exc}")
                        continue
        ...
        return {"status": "COMPLETE", "tier": tier,
                "judgment_complete": (None if _judgment_register_corrupt
                                      else _reg.get("judgment_status") == "COMPLETE"),
                "judgment_pct": (None if _judgment_register_corrupt
                                 else _reg.get("completion_pct", 0)),
                "judgment_register_status": _reg.get("judgment_status", ""),
...
def fig_rggmci_rescue(pairs_csv, png_path, strain_label, plt, logger=None):
    ...
    try:
        with open(pairs_csv, newline="") as f:
            ...
            required = {"bgc_a", "bgc_b", "rggmci_score", "rggmci_confidence"}
            missing = sorted(required - set(reader.fieldnames or []))
            if missing:
                log(f"RESCUE_FIG_INVALID_SCHEMA: missing columns: {','.join(missing)}")
                return None
            for r in reader:
                rows.append(r)
    except (OSError, UnicodeDecodeError, _csv.Error) as exc:
        log(f"RESCUE_FIG_INVALID_SOURCE: {type(exc).__name__}: {exc}")
        return None
    if not rows:
        log("RESCUE_FIG_NO_ROWS: valid RG-GMCI source contains no ranked pairs")
        return None

    # Take top 12 by rggmci_score, split HIGH / MODERATE
    def _score(r):
        return float(r.get("rggmci_score"))

    for index, row in enumerate(rows, start=2):
        try:
            _score(row)
        except (ValueError, TypeError) as exc:
            log(f"RESCUE_FIG_INVALID_SCORE: row {index}: {type(exc).__name__}")
            return None
