...
def _lead_region(value):
    match = re.fullmatch(r"(?:region)?0*(\d+)", str(value or "").strip(), re.I)
    if not match or int(match[1]) < 1:
        raise ValueError("LEAD_IDENTITY_MISSING: explicit region required")
    return f"region{int(match[1]):03d}"


def _lead_identity(strain, inventory, peer=None):
    alias = str(inventory.get("BGC_ID") or "").strip()
    contig = str(inventory.get("Contig") or "").strip()
    if not strain or not alias or not contig:
        raise ValueError("LEAD_IDENTITY_MISSING: strain, full Contig and alias required")
    region = _lead_region(inventory.get("Region") or inventory.get("antiSMASH_Region"))
    for row in (inventory, peer or {}):
        for key in ("Strain", "strain"):
            if row.get(key) and str(row[key]).strip().upper() != strain.upper():
                raise ValueError("LEAD_IDENTITY_CONFLICT: strain")
        for key in ("Contig", "contig"):
            if row.get(key) and str(row[key]).strip() != contig:
                raise ValueError("LEAD_IDENTITY_CONFLICT: contig")
        for key in ("Region", "antiSMASH_Region", "region"):
            if row.get(key) and _lead_region(row[key]) != region:
                raise ValueError("LEAD_IDENTITY_CONFLICT: region")
    return {"strain": strain, "contig": contig, "region": region, "bgc_id": alias,
            "full_identity": f"{strain} / {contig} / {region} / {alias}"}


def build(pkg, bgc, outdir):
    strain = strain_of(pkg)
    inventory_rows = rows(pkg, "_2_inventory.csv")
    inv = {r["BGC_ID"]: r for r in inventory_rows}
    if len(inv) != len(inventory_rows):
        raise ValueError("LEAD_IDENTITY_DUPLICATE: inventory alias")
    if bgc not in inv:
        raise ValueError("LEAD_IDENTITY_UNBOUND: selected locus absent from inventory")
    ...
    identity = _lead_identity(strain, it, tt)
    node, region = identity["contig"], identity["region"]
    full_identity = identity["full_identity"]
    for gene in genes:
        _lead_identity(strain, it, gene)
    ...
    L = []
    W = L.append
    W(f"# {full_identity} — {kname} {page_label}")
    ...
    W(f"**Locus:** {full_identity} · {length_kb} kb · **{boundary}** · **Release:** {g(it,'Release','—')}")
    ...
    if rgg:
        ...
        if partner not in inv:
            raise ValueError("LEAD_IDENTITY_UNBOUND: pair partner absent from inventory")
        partner_identity = _lead_identity(strain, inv[partner])["full_identity"]
        W(f"| Cross-contig linkage (`_4A_RGGMCI`) | best pair {full_identity} ↔ {partner_identity} score {g(r0,'rggmci_score')} ({g(r0,'rggmci_confidence')}, {g(r0,'functional_rescue_class')}) — homology-guided, not a nucleotide join |")
    ...
    filename_identity = "_".join(re.sub(r"[^A-Za-z0-9._-]+", "_", identity[key])
                                 for key in ("strain", "contig", "region", "bgc_id"))
    out = os.path.join(outdir, f"{filename_identity}_LEAD.md")
    ...
    idx = dict(full_identity=full_identity, contig=node, region=region, strain=strain, bgc=bgc, klass=klass, page_label=page_label, lead_tier=tier,
