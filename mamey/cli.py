...
def _phase_receipt(package_dir, phase: str, status: str, **kw) -> None:
    """P0: Append one JSONL receipt row to run_phase_receipts.jsonl.
    Machine-readable phase log for ChatGPT sessions; never raises.
...
def _terminal_mamey_status(validator_status: str, issues: list[str] | tuple[str, ...] | None) -> str:
    ...
    if validator_status.startswith("PASS") and issue_count == 0:
        return "MAMEY_COMPLETE"
    if validator_status.startswith("PASS") and issue_count:
        return "MAMEY_COMPLETE_WITH_ISSUES"
    if validator_status.startswith("FAIL"):
...
def _write_batch_plan(package_dir, strain_id: str, mode: str, source: str) -> None:
    ...
    try:
        (package_dir / f"{strain_id}_BATCH_PLAN.md").write_text(txt, encoding="utf-8")
    except Exception:
        pass


def _write_start_here(package_dir, strain_id, mode, status, raw, interior, edge, fc, corrected, tier):
    ...
    try:
        (package_dir / "START_HERE.md").write_text(txt, encoding="utf-8")
    except Exception:
        pass
...
def _emit_bgc_bank(package_dir, manifest: dict) -> None:
            ...
            if _standard_layout:
                ...
                try:
                    ...
                    (_gold_fig_out / "RUNS_DIR_NOTE.md").write_text(
                        ...
                        f"Used {_runs_dir} as runs_dir instead of {_inferred_runs}; "
...
def _acquire_master_workbook_lock(master_path):
    ...
    unavailable (non-POSIX, or an OSError such as a read-only filesystem) — the run then proceeds
    exactly as unpatched v408 rather than blocking on a lock it cannot take."""
    if not master_path:
        return None
    try:
...
def run_one_strain(
    ...
    try:
        ...
        pass  # sanity check is best-effort; never block a run on it
    ...
    try:
        ...
        pass  # detection is best-effort; never block a run
    ...
    _write_batch_plan(package_dir, strain_id, mode, source)
    ...
    # MAMEY_COMPLETE  = extraction fully passed, package sealed, all gates green
    # MAMEY_COMPLETE_WITH_ISSUES = passed with non-blocking issues (accession label, warnings, etc.)
    # MAMEY_FAILED    = extraction failed (no regions, parse error, gate failure)
    # VALIDATION_FAIL = package written but file-presence or RG-GMCI gate failed
...
def _write_package(run: MameyRun, package_dir: Path,
    ...
    except Exception:  # advisory join; never fail packaging
        pass
    ev_headers = [
    ...
    manifest_data["input_zip_sha256"] = _input_zip_sha256(run.context.input_zip)
    ...
    _emit_bgc_bank(package_dir, manifest_data)  # W6: per-strain bgc_data.json for build_modeb_deepdive
    ...
    try:
        ...
        pass  # non-blocking; triage board CSV is the fallback
...
def run_command(args) -> int:
    ...
    all_pass = bool(results) and all(str(r.get("status", "")) in success_statuses or str(r.get("validator_status", "")).startswith("PASS") for r in results)
...
def chatgpt_init_command(args) -> int:
    """ChatGPT discoverability: surface the ChatGPT operating contract so the assistant
    can prove it located and read the current instruction file before doing work.

    Prints, reading live values straight from CHATGPT_START_HERE.md (never from memory):
      - the path + SHA256 of CHATGPT_START_HERE.md
      ...
      - the inventory of ChatGPT-specific instruction files that exist in this bundle
    ...
    start_here = root / "CHATGPT_START_HERE.md"
    if not start_here.exists():
        emit("✗ CHATGPT_START_HERE.md not found in this bundle — cannot confirm the "
              "ChatGPT operating contract is present.")
        return 1
    ...
    def _extract_chatgpt_gotcha(txt: str) -> str:
        ...
        for line in lines:
            ...
            if capture:
                ...
                if stripped:
                    out.append(stripped)
        return " ".join(out).strip() or "(see CHATGPT_START_HERE.md §3)"
    ...
    emit("◆ SAPOTE–MAMEY · CHATGPT MODE — contract surface")
    ...
    emit(f"   instruction file : CHATGPT_START_HERE.md  {_mark('CHATGPT_START_HERE.md')}  (root bootstrap: 000_READ_ME_FIRST_CHATGPT_CLAUDE.md)", f"   bootstrap contract: bootstrap_contract.yml  {_mark('bootstrap_contract.yml')}", f"   sha256           : {sha}", f"   bundle / engine  : {bundle_engine}", f"   known gotcha     : {gotcha_line}", sep="\n")
...
def doctor_command(args) -> int:
    ...
    if len(gem_have) == len(gem_stack):
        ...
        warn.append(f"Sapote add-ons partial ({len(gem_have)}/{len(gem_stack)}) — "
                    "run install_sapote_addons.sh to enable `mamey compare`")
    else:
        warn.append("Sapote add-ons not installed — run install_sapote_addons.sh "
    ...
    if sci_have:
        ok.append(f"Science stack: {len(sci_have)}/{len(sci)} available "
                  f"({', '.join(sci_have)}) — install_sapote_addons.sh for the rest")
    else:
        warn.append("Science stack (pyhmmer/pyskani/pyfamsa/gb-io) not installed — "
                    "run install_sapote_addons.sh to enable offline HMM search + skani ANI")
    # Companion-files reminder — the anti-'assumed unavailable' guard
    if (BUNDLE_ROOT / "COMPANION_FILES.md").exists():
        ok.append("COMPANION_FILES.md present — companion inputs (antiSMASH ZIPs, genome "
...
def figure_factory_command(args) -> int:
    ...
    except _PhylogenyFigureHold as exc:  # typed refusal from the phylogeny Factory
...
def _flagged_lead_command(args) -> int:
    ...
    try:
        ...
        emit(f"{args.command}: skipped ({type(exc).__name__}: {exc})", file=_sys.stderr)
        return 0
    finally:
        _sys.argv = old
...
_ENGINE_OWNED_TOP_DIRS = frozenset({
    ...
    "Wheelhouse", "wheels", "hooks", "sapote_hooks", "games", "skills", "templates", "resources",
...
def _refuse_outdir_inside_bundle(outdir) -> bool:
    ...
    try:
        ...
        return False
    if full == bundle:
        return _refuse(f"[outdir] {outdir!s} is the engine tree itself ({bundle}); write analysis outputs into a subdirectory or outside the bundle (MAMEY_ALLOW_BUNDLE_WRITES=1 to override)")
    if bundle in full.parents and full.relative_to(bundle).parts[0] in _ENGINE_OWNED_TOP_DIRS:
        return _refuse(f"[outdir] {outdir!s} resolves inside the engine-owned directory {full.relative_to(bundle).parts[0]}/ of the bundle; analysis outputs must not be written into engine directories (MAMEY_ALLOW_BUNDLE_WRITES=1 to override)")
    return False
...
def _start_command(args) -> int:
    ...
    emit(f"Sapote-Mamey bundle {stamp.get('version', '?')} \u00b7 engine {__version__} "
         ...
         "Agents: AGENTS.md (CLAUDE.md is a copy) is the one door; the *_START_HERE files add",
         "        assistant-specific rules only. Trees: docs/PHYLO_AUTOPILOT_WORKFLOW.md.",
         "",
         sep="\n")
...
def build_parser():
    ...
    p = argparse.ArgumentParser(
        prog="mamey",
        description=f"Mamey v{__version__} Standalone ChatGPT Bundle — BGC extraction, batch packaging, and formal marker/cassette registry layer",
    )
    p.add_argument(
        "--version",
        action="version",
    ...
    rf = sub.add_parser("render-figures",
                        help="Render figures from a sealed Mamey package (ChatGPT-safe post-seal step)")
    ...
    ifg = sub.add_parser("interactive-figures",
        help="Emit the AS widget-data aggregate JSON from a directory of sealed packages")
    ...
    chm = sub.add_parser(
        "codex-heatmaps",
        help="Convert Figure Factory matrix CSVs into Codex SVG/HTML publication packs",
    )
    ...
    cfc = sub.add_parser(
        ...
        help="Emit the governed 200-set Codex Figure Factory registry and caption/method catalog",
    )
    ...
    cfs = sub.add_parser(
        "codex-figure-sets",
        help="Render implemented strain/cohort sets from the governed 200-set Codex registry",
    )
    ...
    bgff = sub.add_parser(
        ...
        help="Render the optional eight-view BiG-SCAPE Figure Factory extension",
    )
    ...
    csrc = sub.add_parser(
        ...
        help="Build a provenance-rich figure source bundle from sealed package ZIPs",
    )
    ...
    raf = sub.add_parser(
        ...
        help="Run every applicable figure module against a sealed package "
             "(post-seal aggregate; use after a capped --chatgpt-safe run)",
    )
    ...
    ff = sub.add_parser("figure-factory",
                        help="Render the receipt-bound Figure Factory evidence figure suite "
                             "from a hash-bound JSON config (aggregate or phylogeny kind)")
    ff.add_argument("--config", required=True,
                    help="Path to the hash-bound Figure Factory JSON config "
                         "(schema sapote.figure-factory-next.v2, or a phylogeny figure_kind)")
    ...
    pr = sub.add_parser("phylo-run",
                        help="EXECUTE an APPROVED GToTree->IQ-TREE->sign-off->(fastANI) run "
                             "(tools/run_planned_tree.py). Post-seal, approval-gated, non-blocking.")
    ...
    pa = sub.add_parser("phylo-autopilot",
                        help="16S and/or genome uploads -> routed, gated trees (tools/phylo_autopilot.py). "
                             "Subcommands: plan <dir> | route --query --db --out | run-16s ... --approved-by. "
                             "Post-seal, approval-gated, non-blocking.")
    ...
    st = sub.add_parser("start",
                        help="THE front door: print the five-command happy path for this bundle and run doctor")
    ...
    cgi = sub.add_parser("chatgpt-init",
                         help="Surface the ChatGPT operating contract: version-freshness probe, "
                              "instruction-file inventory, and the documented read-marker")
    ...
    rdp = sub.add_parser("reference-dark",
                         help="Reference-dark novelty prior per BGC (report-only, non-ranking; NEW file, freeze-safe)")
    ...
    mb = sub.add_parser("mode-b",
                        help="Emit Mode B top-lead TRIAGE TABLE + card scaffolds from a sealed package (ChatGPT-safe). "
    ...
    gc = sub.add_parser(
        "compare",
        help="Gemini two-strain comparison: per-BGC gene-level similarity + comparator recovery")
    ...
    # run order Sapote_Mamey_ROADMAP.md documents ("KCB front page -> scanners on every
    ...
    # despite biopython being documented optional (PREREQUISITES.md §0; parsers.py shims it via
        ...
        try:
            ...
            if e.name == "Bio":
                raise SystemExit(
                    ...
                    "(see PREREQUISITES.md §3). All other "
    ...
    ex = sub.add_parser("triage-raw",
                         help="Guided PRE-EXTRACTION genome triage: KCB front page + scanner "
                              ...
                              "(see Sapote_Mamey_ROADMAP.md). For sealed-package novelty/"
    ...
    bp.add_argument("--json-evidence", default="off", dest="json_evidence", choices=["off", "bounded", "full"],
                    help="antiSMASH JSON mode for BGC parsing (default: off, ChatGPT-safe)")
...
def main(argv=None) -> int:
    ...
    if hasattr(args, "strains") and hasattr(args, "input_zip"):
        if args.strains and args.input_zip:
            emit("Error: use either --strains (batch) or --input-zip (single), not both.")
            return 1
        if getattr(args, "accession", None) and not args.input_zip:
            emit(json.dumps({
            ...
            return 1
        if not args.strains and not args.input_zip and args.command == "run":
            emit("Error: provide --input-zip (single strain) or --strains (batch).")
            return 1
