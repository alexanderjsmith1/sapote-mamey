...
def _triage_row_for(package_dir: str | Path, bgc_id: str) -> dict:
...
def _strain_compound_names(package_dir: str | Path) -> set:
...
def record_mode_b(
    ...
    try:
        ...
        _cnames = _strain_compound_names(package_dir)
    ...
    try:
        ...
        _trow = _triage_row_for(package_dir, bgc_id)
        if _trow:
    ...
    _update_register(package_dir, bgc_id, session_id, str(mb_path), verdict)

    return {"path": mb_path, "verdict": verdict}
...
def record_batch_complete(
    ...
    _recalculate_and_write(package_dir, reg)
    return reg
...
def read_register(package_dir: str | Path) -> dict[str, Any]:
    ...
    try:
        ...
        if last_good.exists():
            try:
                return json.loads(last_good.read_text(encoding="utf-8"))
            except Exception:
                pass
...
def compile_ready(package_dir: str | Path, ranked_bgc_ids: list[str] | None = None) -> tuple[bool, str]:
    ...
    if isinstance(_reg_status, str) and _reg_status.startswith("CORRUPT"):
...
def _recalculate_and_write(package_dir: str | Path, reg: dict[str, Any]) -> None:
    ...
    recovering_from_unreadable_register = reg.get("judgment_status") == "CORRUPT"
    ...
    _sync_judgment_banner(package_dir, reg)
...
def _sync_judgment_banner(package_dir: str | Path, reg: dict) -> None:
    ...
    try:
        ...
        pass  # never fail the register write over a banner update
