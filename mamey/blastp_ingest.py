...
def _current_locus_lengths(package: Path) -> dict[str, dict[str, int]]:
    ...
    contexts = sorted(package.glob("*gene_context.jsonl"))
    if len(contexts) > 1:
    ...
    if contexts:
        ...
        tables = sorted(package.glob("*gene_by_gene_all_bgcs.csv"))
        if len(tables) > 1:
...
def ingest_blastp_trove(package: str | Path, trove_dir: str | Path, channel: str,
    ...
    strain_dirs = [troot / package_strain] if (troot / package_strain).is_dir() else \
                  [d for d in troot.iterdir() if d.is_dir()] if not any(troot.glob("BGC*")) else [troot]
    written: dict[str, int] = {}
    rejected: list[dict] = []
    sources: list[dict] = []
    plans: list[tuple[str, Path, list[dict]]] = []
    for sdir in strain_dirs:
        for bgc_dir in sorted(p for p in sdir.iterdir() if p.is_dir() and p.name.upper().startswith("BGC")):
            bgc = bgc_dir.name
    ...
    for bgc, _src, new_row_list in plans:
        ...
        if merged:
            ...
            _atomic_write_text(opath, _buf.getvalue())
        elif opath.exists():
...
def _update_ingest_ledger(store_dir: Path, channel: str, written: dict,
    ...
    try:
        data = json.loads(led.read_text(encoding="utf-8")) if led.exists() else {}
...
def install_channel_top10(package: str | Path, trove_dir: str | Path, channel: str,
    ...
    for sdir in strain_dirs:
        for bgc_dir in sorted(p for p in sdir.iterdir() if p.is_dir()
            ...
            for pat in _TOP10_GLOBS.get(channel, ("*_blastp_top10*.csv",)):
                pat = pat.format(bgc=bgc)
                cand = sorted(bgc_dir.glob(pat)) if "*" in pat else \
    ...
    for bgc, rows in plans:
        ...
        _atomic_write_text(store / f"{bgc}_top10.csv", _buf.getvalue())
...
def blastp_status(package: str | Path) -> list[dict]:
    ...
    rows = []
    bgcs = sorted({p.name.split("_online_blastp")[0] for p in outdir.glob("*_online_blastp.csv")}) if outdir.is_dir() else []
    for bgc in bgcs:
...
def _domains_by_locus(pkg: Path) -> dict[str, str]:
    ...
    Fail-open: a missing/unreadable gene_context yields {} and the column stays blank.
    """
    out: dict[str, str] = {}
    gc = next(iter(sorted(pkg.glob("*gene_context.jsonl"))), None)
    if gc is None:
        return out
    try:
...
def _self_binomial(package) -> str | None:
    ...
    try:
        man = _json.loads((_P(package) / "manifest.json").read_text(encoding="utf-8"))
    except Exception:
        return None
...
def _is_self_hit(sciname, pct_identity, self_binomial, hit_rank=None) -> bool:
    ...
    if sciname:
        ...
        if f"{toks[0]} {toks[1].strip('.,')}".lower() != str(self_binomial).lower():
            return False
        try:
            return float(pct_identity) >= 99.0
        except (TypeError, ValueError):
            return False
    ...
    if hit_rank is None:
        return False
    try:
        return int(hit_rank) == 1 and float(pct_identity) >= 99.0
    except (TypeError, ValueError):
        return False
...
def write_nr_overlay(package: str | Path, rows: list[dict]) -> dict:
    ...
    def _bits(rec: dict) -> float:
        try:
            return float(rec.get("bitscore") or 0)
        except (TypeError, ValueError):
            return 0.0
    ...
    for bgc, slot in per_bgc.items():
        ...
        if path.exists():
            try:
                ...
                pass   # unreadable prior overlay: prefer the fresh rows, never crash
    ...
    for bgc, slot in per_bgc.items():
        ...
        w.writeheader()
        w.writerows(recs)
        _atomic_write_text(outdir / f"{bgc}_online_blastp.csv", _buf.getvalue())
...
def _manual_binding_guard(package: Path, strain: str, rows: list[dict], csv_path: Path) -> tuple[list[dict], dict]:
    ...
    try:
        ...
        if (any(package.glob("*gene_context.jsonl"))
                or any(package.glob("*gene_by_gene_all_bgcs.csv"))):
            raise
...
def _existing_b5_keys(ws) -> set:
    """Keys already present in an open B5 worksheet. Fail-open: an unreadable or
    header-less sheet yields an empty set, which restores the old append behaviour
    rather than dropping rows. v9.7.241 (P7c)."""
    try:
