...
def _is_large_bgc(bgc_context: Optional[dict]) -> bool:
    ...
    try:
        if int(bgc_context.get("total_domains", 0)) >= 40:
            return True
        if float(bgc_context.get("length_kb", 0) or 0) >= 30:
            return True
    except (TypeError, ValueError):
        pass
...
# AB or AF scores are routing priors; they do not imply a named assay, activity, or BGC linkage.
    ...
    if explicit_am is None:
        try:
            ab = float(ctx.get("ab_score") or ctx.get("AB_auto") or 0)
        except (TypeError, ValueError):
            ab = 0.0
        try:
            af = float(ctx.get("af_score") or ctx.get("AF_auto") or 0)
        except (TypeError, ValueError):
            af = 0.0
    ...
    try:
        n_hp = int(ctx.get("strain_high_priority_count") or 0)
    except (TypeError, ValueError):
        n_hp = 0
    ...
    def _count(*keys) -> int:
        """Typed integer parse; a non-numeric or sentinel value is 0, never 'truthy'."""
        for k in keys:
            v = ctx.get(k)
            if v is None:
                continue
    ...
    has_assembly_line = (_count("module_count", "n_modules") > 0
    ...
    has_measured_assembly_line = (_count("module_count", "n_modules") > 0
                                  or _flag("has_assembly_line"))
    multi_protocluster = _count("protocluster_count", "n_protoclusters") > 1 \
    ...
    has_cohort_context = _flag("cross_strain_identity", "identity_comparison_bound") \
        or _count("n_cross_strain_comparisons") > 0
    ...
    if qualified:
        try:
            ...
            has_gcf_assignment = False
    has_4d_rows = _count("n_4d_rows") > 0 or _flag("has_4d_rows", "two_proof_rows_bound")
    has_4a_rows = _count("n_4a_rows") > 0 or _flag("has_4a_rows", "rggmci_rows_bound")
