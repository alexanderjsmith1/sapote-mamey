...
def render_figures_command(args) -> int:
    ...
    def _label(row: dict) -> str:
        if node_first:
            node = row.get("Contig", row.get("Node_ID", "")).split(".")[0]
            bgc = row.get("BGC_ID", "")
            return f"{node} ({bgc})" if node else bgc
        return row.get("BGC_ID", "")
