...
def _parse_engine_version(pkg: Path) -> tuple[int, ...] | None:
    ...
    if not man.exists():
        return None
    try:
        wv = json.loads(man.read_text(encoding="utf-8")).get("workflow_version", "")
    except Exception:
        return None
...
def _package_mode(pkg: Path) -> str:
    """Read the run mode from manifest.json ('smoke' | 'gold' | ...). '' if unreadable."""
    man = pkg / "manifest.json"
    if not man.exists():
        return ""
    try:
        return str(json.loads(man.read_text(encoding="utf-8")).get("mode", "")).lower()
    except Exception:
        return ""
