#!/usr/bin/env bash
# install_sapote_addons.sh — install the Sapote add-ons stack (wheels + HMM + NP Atlas data) from vendored
...
#   * an explicit path                  (bash install_sapote_addons.sh /path/to/wheels)
#
...
if [ "$found" -eq 0 ]; then
  ...
  echo "  bash install_sapote_addons.sh /path/to/wheels" >&2
  exit 1
fi
