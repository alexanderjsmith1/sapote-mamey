...
`PREREQUISITES.md`). Their code is **not** included in this repository — it is installed via `pip`
...
These programs are installed by the operator via conda/bioconda and the pipeline invokes them as
**separate processes** (no source vendored, no linking). Under standard "mere aggregation," their
copyleft licenses do **not** reach the MIT bundle. Listed for transparency; see
`docs/EXTERNAL_TOOL_INVENTORY.md` for versions and citations. Each project's own license file
governs — confirm against the version you install.
...
| **GToTree** | core-genome phylogenomics backbone | GPL-3.0 |
| **Prodigal** | gene prediction | GPL-3.0 |
| **trimAl** | alignment trimming | GPL-3.0 |
| **IQ-TREE** | maximum-likelihood trees | GPL-2.0 |
| **FastTree** | fast draft trees | GPL-2.0 |
| **HMMER** | profile-HMM search | BSD-3-Clause |
| **MUSCLE** | multiple sequence alignment | GPL-3.0 (v5; verify per installed version) |

Because none of these are bundled or linked, their copyleft terms impose no obligation on this
MIT-licensed repository — this is standard aggregation, no conflict. (The one place a GToTree
*derivative* — a source patch — does sit in-tree is tracked separately as a release-hygiene item;
that is outside this pip/invoked-tool inventory.)
