...
**The CCTT trigger set is representative.** With 8 of 18 trigger families firing across the 67 clusters, the run exercises most of the framework. A pipeline update to any trigger family in {HAL, PHO, BLA, ENE, LAN, LASSO, DKP, TET} can be regression-tested against the spectabilis run's expected trigger calls.
