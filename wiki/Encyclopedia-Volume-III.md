...
- **Diagnostic bonus (+25).** When a corroborated class-defining trigger fires, the relevant axis gets a fixed
  bonus — a nucleoside, HSAF/PTM, or polyene-macrolide trigger (T43-NUC, T43-PTM, T43-PYE) lifts **AF**; a lanthipeptide/lasso/thioamide
...
| Novelty corrections | KCB cumulative `> 10000` → `−15` (strong known similarity = less novel); missing/UNKNOWN_KCB → no novelty credit; RiQ `< 0.5` → `+10` |
...
7.  **Novelty corrections** — observed KCB `>10000` −15 / missing or unknown KCB no credit; RiQ `<0.5` +10.
...
*End of Volume III. Next: Volume IV — Detection & Trigger Frameworks (the CCTT T43 framework and its eighteen
