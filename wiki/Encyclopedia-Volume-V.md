...
an observed high known-cluster signal **subtracts** novelty (`kcb_cumulative > 10000 → novelty − 15`). Missing or UNKNOWN_KCB evidence supplies **no novelty credit**; the former +5 absence bonus was removed. A measured low RiQ (`< 0.5`) adds
...
- **Diagnostic bonus.** A corroborated diagnostic trigger adds `DIAGNOSTIC_BONUS = 25` to its axis. The
  antifungal diagnostics are **`{T43-NUC, T43-PTM, T43-PYE}`**; the antibacterial diagnostics are the **eight**
  **`{T43-LAN, T43-LASSO, T43-THA, T43-PHO, T43-AMC, T43-BLA, T43-GPA, T43-BLT}`** (PHO/AMC/BLA wired v9.7.20/.21, each firing only
