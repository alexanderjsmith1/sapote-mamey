...
3.  **AB/AF diagnostic subset.** A few families are strong enough bioactivity diagnostics to move a score directly
    (→ §III.3). The antifungal pair is **T43-NUC** and **T43-PTM**. The antibacterial set is **six**:
    **T43-LAN**, **T43-LASSO**, **T43-THA**, plus **T43-PHO** (phosphonate antibiotics), **T43-AMC** (aminoglycoside),
    and **T43-BLA** (β-lactam) — the last three wired in v9.7.20/.21 because each fires only on its *own* biosynthetic
...
The contrast is the chapter in miniature. **T43-THA** is one of the six AB diagnostics: corroborated on a RiPP, it
...
## §IV.3 · The fifteen CCTT trigger families

The engine's `CCTT_PATTERNS` carries exactly **fifteen** gene-signature trigger families (`source_scans.py`). Each
...
**Not CCTT trigger families (don't confuse with the fifteen above).** Three names share the `T43-`/detection
