...
| AB / AF diagnostic sets | `scoring.py` `AB_DIAGNOSTIC_TRIGGERS` (6) / `AF_DIAGNOSTIC_TRIGGERS` (2) | LAN/LASSO/THA/PHO/AMC/BLA · NUC/PTM | which triggers are strong enough to floor a tier |
...
| CCTT triggers / markers | `source_scans.py` `CCTT_PATTERNS` + the registry (→ §VIII.3) | 15 families | the class-corroboration signatures |
...
| Novelty corrections | `scoring.py` (novelty corrections) | KCB \>10000 −15 · KCB-dark +5 · RiQ \<0.5 +10 | how known-similarity and rarity move novelty |
...
| Primary-metab over-call set | `scoring.py` `WEAK_OVERCALL_CLASSES` | 12 weak labels (other, fatty_acid, t2pks/t3pks, terpene, saccharide, arylpolyene, …) | which "class" labels are weak enough that a housekeeping core triggers the primary-metab guard |
