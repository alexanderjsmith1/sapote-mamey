...
what EPA-ng, pplacer, and SEPP were built for, and it is exactly the design the Developer or User proposed
...
3. **`place`** — align the lab 16S into the reference coordinate system (`mafft --add --keeplength`, or
   `hmmalign` via `--fragmentary` for short Sanger reads) → **epa-ng** → `epa_result.jplace`.
