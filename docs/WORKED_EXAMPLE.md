...
- **Phylogenomics.** A tree needs a panel — see `docs/GTOTREE_WORKFLOW.md`. Note the practical floor:
  ...
  an explicitly selected governed registry (`OUTGROUP_REGISTRY`). The shared resolver exposes
  the bundled reference snapshot for inspection, but it does not treat that snapshot as project authority.
