...
### Evidence work is not interchangeable with evidence availability

For each required stream distinguish retrieved records, an inspected inventory, and completed
functional adjudication. A source file's existence, a populated table, a count match, or a saved
detector verdict does not demonstrate that the author evaluated its effect on the focal model.
Accessible but unread material is unfinished authoring work, not unavailable biological evidence.
Record an explicit hold until the required review is complete; do not conceal that hold by
changing a section to not-applicable.

### Partner review requirements

An RG-GMCI partner inventory may show exact identities, ranks and source marker calls without
claiming functional review. To describe a partner as functionally reviewed, record:

1. the specific focal gene, reaction, or continuation question being evaluated;
2. the exact partner genes and source-bound architecture relevant to that question;
3. compatible and conflicting evidence, including substrate/carrier requirements, catalytic
   checks and assembly geometry where relevant;
4. the strongest alternative (for example a paralog, an independent pathway or generic
   shared-reference signal); and
5. a bounded disposition and discriminating next observation or test.

Retain saved detector labels separately from the author's adjudication. A saved rescue class
plus a general disclaimer is an inventory summary, not a mechanistic judgment. Explicitly report
which partners remain unreviewed. Sampling five leading partners does not review the remaining set.

Do not silently truncate genes or domain labels in a display described as complete. A compact
summary must state its selection rule, displayed versus available counts, omissions and a locator
for the full source. An alphabetically ordered set of marker names is not an ordered domain
architecture; reconstruct architecture from the appropriate coordinate-bound source instead.
Printing every marker hit also does not, by itself, establish functional interpretation.

### Census reconciliation

Count domain instances, module objects and module-bearing proteins separately. Reconcile numerical
totals against the exact source roster, including starter objects and domains outside module
objects. Preserve source flags (such as complete or starter) separately from interpretive
alternatives. A source-labelled starter lacking a KS cannot silently become a failed extension
module. If the author disputes the source classification, state the evidence and uncertainty;
contig-edge proximity alone does not settle the biological interpretation.

### Section assessments and independent audit

Assess each section against its own scientific requirements with an identifiable evidence basis.
Neither uniformity nor diversity of section states proves those assessments correct. Uniformity
may prompt review, but must not automatically fail a legitimate assessment; inserting one different
state must not clear a substantive defect. Do not impose a minimum number of different states.

Mechanical checks report their actual scope (for example section structure, roster binding, or
table widths). They cannot certify functional review by checking for a phrase, a nonempty cell,
state diversity, or absence of duplicate paragraphs. Author self-audit and independent review
remain separately attributed verdicts on identified saved bytes.

This clarification does not introduce a new runtime enum. If the implemented vocabulary cannot
represent a required hold, record that incompatibility and withhold a gate-clean claim; do not
force a misleading existing state. A documentation change alone does not implement a validator.
