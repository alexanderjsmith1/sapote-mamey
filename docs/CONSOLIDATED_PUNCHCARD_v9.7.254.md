...
4. **S6 — doc.** `pandas` half-documented in `PREREQUISITES.md`. `S6_prereq_pandas.diff` applies
   clean (-p0). PENDING.
...
| **S6** `S6_prereq_pandas.diff` | `PREREQUISITES.md` | clean `-p0` | **PENDING (doc)** | pandas still `*(optional)*` at L61; no wheels/find-links entry |
