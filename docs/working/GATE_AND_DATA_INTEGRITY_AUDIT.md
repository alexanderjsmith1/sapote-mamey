...
Referenced only in `SESSION_START_MANIFEST.md`, run by nothing automatically. It checks tier *sameness* (versions/cassettes/markers identical across tiers) — which, recall, is why the v9.7.91 version desync passed parity. It's a useful post-cut tool but isn't a wired gate. Recommend either wiring it into the build (it needs all four tier zips, so it runs *after* the four cuts) or formalizing it as a documented manual release step.
