# Mamey v1.9.163 Standalone ChatGPT Bundle
...
Mamey v1.9.163 is standalone **inside ChatGPT** when the user uploads this bundle plus antiSMASH ZIPs. The workflow does not depend on a previously installed Mamey package or on hidden project state. The local `mamey_run.py` launcher forces this bundle's `mamey/` package to be used first.
