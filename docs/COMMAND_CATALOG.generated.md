...
*Generated from the live argparse tree of `mamey.cli` by `tools/gen_command_catalog.py` — bundle v9.7.414 · engine 1.9.152. Do not edit by hand; `--check` fails the build when stale.*
...
| `start` | THE front door: print the five-command happy path for this bundle and run doctor |
...
_Render from a sealed package; never changes a score._

| command | what it does |
|---|---|
| `render-figures` | Render figures from a sealed Mamey package (ChatGPT-safe post-seal step) |
| `figure-factory` | Render the receipt-bound Figure Factory evidence figure suite from a hash-bound JSON config (aggregate or phylogeny kind) |
| `figures` | Publication figures (diagram | atlas | ani | gcf-network | clinker) |
| `interactive-figures` | Emit the AS widget-data aggregate JSON from a directory of sealed packages |
...
| `render-all-figures` | Run every applicable figure module against a sealed package (post-seal aggregate; use after a capped --chatgpt-safe run) |
...
| `codex-figure-catalog` | Emit the governed 200-set Codex Figure Factory registry and caption/method catalog |
| `codex-figure-sets` | Render implemented strain/cohort sets from the governed 200-set Codex registry |
| `codex-figure-sources` | Build a provenance-rich figure source bundle from sealed package ZIPs |
| `codex-bigscape-figure-sets` | Render the optional eight-view BiG-SCAPE Figure Factory extension |
| `codex-heatmaps` | Convert Figure Factory matrix CSVs into Codex SVG/HTML publication packs |
...
| command | what it does |
|---|---|
| `mode-b` | Emit Mode B top-lead TRIAGE TABLE + card scaffolds from a sealed package (ChatGPT-safe). NOT a finished 48-section Mode B card: finished cards are authored per wiki/Mode-B-Gene-First-and-48-Section-Manual.md and verified with `verify-modeb` (v9.7.405 wording). |
...
| `reference-dark` | Reference-dark novelty prior per BGC (report-only, non-ranking; NEW file, freeze-safe) |
...
| `genus-appendix` | Antifungal/antibacterial candidate appendices, grouped by genus (report-only) |
...
| `discover` | Orient in a workspace: list packages, coverage, and next actions |
| `discovery` | (no description) |
| `explore` | question-driven genome exploration (divergence / co-capture / lead board) |
| `compare` | Gemini two-strain comparison: per-BGC gene-level similarity + comparator recovery |
...
_Existing-output consumers; no tree is inferred here._

| command | what it does |
|---|---|
| `phylo-run` | EXECUTE an APPROVED GToTree->IQ-TREE->sign-off->(fastANI) run (tools/run_planned_tree.py). Post-seal, approval-gated, non-blocking. |
| `phylo-autopilot` | 16S and/or genome uploads -> routed, gated trees (tools/phylo_autopilot.py). Subcommands: plan <dir> | route --query --db --out | run-16s ... --approved-by. Post-seal, approval-gated, non-blocking. |
...
| `triage-raw` | Guided PRE-EXTRACTION genome triage: KCB front page + scanner evidence + rare-motif scan + split-detector + bgc_walk on the priority regions, in one pass, directly on a raw antiSMASH ZIP/dir (see Sapote_Mamey_ROADMAP.md). For sealed-package novelty/divergence correctness checks after extraction, see `mamey explore`. |
| `chatgpt-init` | Surface the ChatGPT operating contract: version-freshness probe, instruction-file inventory, and the documented read-marker |
| `wheelhouse` | Manage the Wheelhouse lab-data store (strains, scanners, validations) |

_115 commands catalogued._
