...
This affected every file that sync_version patches: TAG, docs/BUNDLE_CAPABILITIES.md, RELEASE_MANIFEST.md, docs/PLAYBOOK.md, prompts, and the AGENTS.md bootstrap files. The docs/PLAYBOOK.md had two rules with a different syntax (`[\d._]+`) that also did not match letters, causing a separate loop.
...
- AGENTS.md: soft guideline replaced with hard rule
...
| v9.7.132–134 | Cross-assistant bootstrap | AS-XXX SPAdes contig fix; `AGENTS.md`; `AGENTS.md`; smoke-first default |
...
The `AGENTS.md` run command block showed `pip install` commands without a prominent "this works offline" callout, so the audit chat concluded the commands would fail.
...
**Fix:** `AGENTS.md` now prominently states before the run commands:
