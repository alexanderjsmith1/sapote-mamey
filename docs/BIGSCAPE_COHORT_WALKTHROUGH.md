...
`bigscape_prep.py --drop-contigs STRAIN=removed_contigs.tsv` drops that strain's regions on contigs a decontamination removed (first TSV column = antiSMASH record id); repeat per strain. Use it whenever the antiSMASH registry marks a zip `RAW_CONTAMINATED_ASSEMBLY`, as it does for AS-XXX. Dropped regions are listed in `DROPPED_CONTIG_REGIONS.tsv`. A drop list naming a strain that did not stage exits 2, so a mistyped id cannot silently drop nothing.

An empty or duplicate drop list is refused. A list with zero matching regions also exits 2; after verifying that the assembly and contig IDs agree and zero regions are genuinely affected, pass `--allow-zero-drop STRAIN` for that exact strain. The zero count remains visible in stderr and the dropped-region table.
