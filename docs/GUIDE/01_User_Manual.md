...
*Operating guide for the Sapote–Mamey genome-mining pipeline · current to bundle v9.7.414 / engine Mamey 1.9.152*
*· 2026-06-29*
...
*(engine 1.9.152, bundle v9.7.414)*
...
```bash
bash install_sapote_addons.sh            # auto-discovers every attached add-on part
bash install_sapote_addons.sh /path/to/wheels   # or point it at an explicit directory
```
...
python3 tools/sync_version.py --check # should report `engine 1.9.152, bundle 9.7.414`
...
→ Full dependency reference and system binary install (pandoc, xelatex): `PREREQUISITES.md`.
...
New in v9.7.90. Deterministic gene-arrow maps render automatically in-run into the package's `locus_maps/` directory, catalogued in `locus_maps/locus_map_manifest.csv`. Each gene is an arrow drawn to bp scale, strand-aware, colored by its functional role (PepM, Ppd, NRPS module, PKS module, RiPP machinery, transport, regulation, …) from `mamey/data/locus_role_palette.json`; unrecognized genes are grey "other / hypothetical". Three triggers fire automatically:
...
*Sapote–Mamey User Manual · current to bundle v9.7.414 / engine Mamey 1.9.152 Consolidates the former 01_User_Guide.md and 01_User_Manual.html into one task-flow-first operating manual; deep internals live in the Encyclopedia, term definitions in GLOSSARY.md.*
