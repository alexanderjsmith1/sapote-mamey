...
**Version:** v9.7.414 / engine Mamey 1.9.152
...
```bash
unzip sapote-mamey-v9.7.414-CODE-20260907v97414a.zip
cd sapote-mamey-v9.7.414-CODE-20260907v97414a
...
python3 tools/sync_version.py --check # → engine 1.9.152, bundle 9.7.414
```

→ Full dependency reference: `PREREQUISITES.md`
...
## 10. Post-seal deliverable subcommands (v9.7.338)
...
## What's new in v9.7.338

- **Twelve new post-seal subcommands/deliverables** — see §10 above: `cohort-leads`, `cohort-assemble`,
  `comparator-coverage`, `af-dossier`, `good-guesses`, `modeb-export`, `figures kcb-locusmap`,
  `domain-reference`, `realistic-count`, `novelty-shortlist`, `signoff`, and `verify-modeb --interp`.
  All sign-off gated; non-scoring unless noted.
- **§4 Mode-B evidence gate now bites (MB-01)** — the §4 evidence-grid check is enforcing, not advisory.
- **NAPAA standing rule follows the registry (RG-01)** — the NAPAA exclusion is now driven by
  `rules_registry.json` rather than a hardcoded pattern (same behaviour, single source of truth).
...
## What's new in v9.7.147

- **Edge/FC BGC equality** — all detected BGCs visible in triage sorted by score, not boundary status
- **§28 + §30 required** — evidence ledger and decision tree for every completed Mode B card
- **ChatGPT Execution Slice** — `CHATGPT_EXECUTION_SLICE_v97147.md` replaces the slim kernel; no abbreviated defaults
- **Post-MAMEY_COMPLETE handback** — figures, strain brief, locus maps presented automatically
- **Packaging seal fix** — checksums correctly exclude mutable post-seal files; `mamey validate` verifies SHA256 values
- **Mamey-first gate** — Mode B refused without sealed package on both Claude and ChatGPT sides
- **Engine lineage governance** — `ENGINE_LINEAGE.md` + `sync_version --check` enforcement
