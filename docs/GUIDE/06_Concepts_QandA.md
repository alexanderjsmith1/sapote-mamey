...
*A running concepts companion to the GUIDE. the Developer or User asks, Claude answers in the voice we actually use — direct, honest about what's observed vs. inferred, and not stiff about it. Useful first, entertaining second; we'll take both. This is the conversational "why does it work this way" layer — clipped definitions live in the canonical [`GLOSSARY.md`](../GLOSSARY.md) (see its **Core concepts** section for the load-bearing terms), and the deep internals live in the Encyclopedia (`03_Technical_Manual_Encyclopedia.html`).*

*Current to bundle v9.7.428 / engine Mamey 1.9.163.*
...
*Numbered by position in the 100-question bank so they map straight back to source. Same voice, dialed a little tighter since these come in batches. Definitions referenced here live in the canonical [`GLOSSARY.md`](../GLOSSARY.md).*
...
*Sapote–Mamey Concepts Q&A · current to bundle v9.7.428 / engine Mamey 1.9.163 A running document — concepts are version-stable; inline version references (e.g. "shipped in v9.7.91", "v9.7.97 CODE tier") are historical record, not a currency claim. Companion to `../GLOSSARY.md` (definitions) and `03_Technical_Manual_Encyclopedia.html` (deep internals); Q&A is the conversational "why does it work this way" layer.*
