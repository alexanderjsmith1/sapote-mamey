...
Reference menu of tools, modes, and deliverables. Begin with `AGENTS.md` for assistant operation; consult this catalog as needed. Engine: Mamey v1.9.167 · Bundle: sapote-mamey-v9.7.435.
...
**Directory map:** `mamey/` (engine + `_vendor/ijson`) · `tools/` (387 scripts, §2) · `prompts/` (+`reuse/`, §4) · `docs/` (reference, `modules/`, `troubleshooting/`, `standalone/`, `legacy/`) · `cohort/` · `offline_deps/` · `examples/` · `templates/` · `deliverables/` (+`reports/`).
...
### Tools inventory (387 scripts in `tools/`)
