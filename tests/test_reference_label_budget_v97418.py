"""Label regression_418 — reference tip labels lost the strain NUMBER of a type strain.
...
def test_a_doubled_accession_no_longer_eats_the_strain_number():
    """v9.7.417 rendered this real outgroup tip as 'Pseudonocardia thermophila ATCC (NR_118886.1)'."""
    lab = _mod()._ref_label(DOUBLED, "Saccharothrix")
    assert "19285" in lab, f"strain number lost: {lab}"
    assert lab.count("NR_118886") == 1
    assert "NR 118886" not in lab, f"a second raw accession survived into the text: {lab}"
