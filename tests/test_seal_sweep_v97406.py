...
def test_sweep_doc_surface_clean_tree_has_no_findings(tmp_path):
    bundle = _write_bundle(
        tmp_path,
        changelog="# v9.7.406 · current\nclean\n",
        start_here_files={"AGENTS.md": "current bundle: v9.7.406, all clear\n"},
...
def test_sweep_start_here_version_citation_flags_missing_citation(tmp_path):
    bundle = _write_bundle(
        ...
        changelog="# v9.7.406 · current\nclean\n",
        start_here_files={
            "AGENTS.md": "current bundle: v9.7.406\n",
            "docs/FIGURES_START_HERE.md": "no version line in here at all\n",
        },
        build_stamp_version="9.7.406",
    )
    findings = seal_sweep.sweep_start_here_version_citation(bundle)
    assert findings == [{
        "path": "docs/FIGURES_START_HERE.md", "code": "VERSION_STALE",
...
def test_run_sweep_clean_bundle_has_no_findings(tmp_path):
    bundle = _write_bundle(
        tmp_path,
        changelog="# v9.7.406 · current\nseal-ready\n",
        start_here_files={"AGENTS.md": "current bundle: v9.7.406\n"},
...
def test_run_sweep_dirty_bundle_reports_every_check_family(tmp_path):
    bundle = _write_bundle(
        tmp_path,
        changelog="# v9.7.406 · current\nTO BE FILLED\n",
        start_here_files={"AGENTS.md": "no citation here\n"},
        extra_docs={"README.md": "PENDING, not yet in this tree\n"},
        build_stamp_version="9.7.406",
    )
...
def test_live_tree_sweep_is_clean():
    """This tree's own real root must sweep clean -- not merely sweep without crashing.

    v9.7.416. The earlier form of this test called run_sweep(ROOT) and asserted only the SHAPE of
    the findings, on the stated ground that "a stale hardcoded expectation would itself be the kind
    of drift this sweep exists to catch". That reasoning is right about *specific* findings and
    wrong about the empty set: `findings == []` hardcodes nothing, it is the invariant the sweep
    exists to assert. With only the shape asserted, the sweep reported VERSION_STALE on
    docs/FIGURES_START_HERE.md and the suite stayed green -- and that file shipped citing
    v9.7.408 / engine 1.9.148 in the .413, .414 and .415 seals (checked against all three sealed
    zips). No release script runs `seal_sweep --check`, so this test is the only place the sweep
    can actually gate anything.

    A failure here is not flakiness: it names the file and the reason. Fix the file, or -- if the
    sweep itself is wrong about it -- fix the sweep. Do not re-weaken this assertion.
    """
    ...
    assert findings == [], (
        "seal_sweep reports findings against this tree; the seal would ship them:\n  "
