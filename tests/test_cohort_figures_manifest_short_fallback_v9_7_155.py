...
def _make_pkg(runs, sid, with_manifest_short=False, with_manifest=True):
    ...
    if with_manifest_short:
        json.dump({"taxonomy": "Micromonospora sp.", "assembly_tier": "MODERATE",
                   "raw_bgcs": 7}, open(f"{pkg}/manifest_short.json", "w"))
    return pkg
...
def test_no_manifest_at_all_degrades_to_defaults():
    """Even with neither manifest_short nor manifest, no crash — safe defaults so
    downstream int()/sort never throws."""
    with tempfile.TemporaryDirectory() as d:
        pkg = _make_pkg(d, "AS-901", with_manifest_short=False, with_manifest=False)
        ms = _load_manifest_short(pkg, "AS-901")
        assert ms["raw_bgcs"] == 0
        assert ms["assembly_tier"] == "UNKNOWN"
