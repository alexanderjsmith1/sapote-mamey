...
DETECTION_PATTERN_FILES = {
    ...
    "tools/public_release_audit.py",  # v9.7.381 leak detector: contains the patterns it hunts for
    "tests/public/test_public_release_v9_7_381.py",       # v9.7.381: detection regex + planted-leak fixtures
    "tests/public/test_release_audit_fails_closed_v9_7_381.py",  # v9.7.381: plants leaks to prove fail-closed
    "tests/test_audit_modeb_support_card.py",
    # The portability guards themselves must contain the literal in order to search for it.
    ...
    # v9.7.367 (A10/DEC-02): the SINGLE sanctioned home of the historical default — every other
    # site now resolves through workspace_root(); this module and its test necessarily hold the literal.
    "mamey/workspace_root.py",
...
def test_ci_workflow_present_and_runs_the_real_gates():
    ...
    text = wf.read_text()
    assert "pytest tests/" in text, "CI must run the suite"
