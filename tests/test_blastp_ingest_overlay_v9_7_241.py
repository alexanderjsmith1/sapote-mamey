...
def test_p7b_corrupt_gene_context_is_a_hold_not_blank_domains(tmp_path):
    """STRAIN-TEST / NODE_2_length_5000_cov_25 / region001 / BGC017 cannot lose domain evidence."""
    from mamey.blastp_ingest import write_nr_overlay
    pkg = tmp_path / "package"
    pkg.mkdir()
    (pkg / "STRAIN-TEST_gene_context.jsonl").write_text("{not-json\n", encoding="utf-8")
    with pytest.raises(ValueError, match="BLASTP_GENE_CONTEXT_HOLD"):
        write_nr_overlay(
            pkg,
            [_row("BGC017", "ctg2_360", 4071, "WP_2", "some protein", "96.5")],
        )
...
def test_p7c_headerless_or_odd_sheet_is_a_schema_hold(tmp_path):
    """A present malformed B5 sheet cannot accept rows without a header or dedupe contract."""
    from mamey.blastp_ingest import BlastpStoreSchemaError, _existing_b5_keys
    wb = openpyxl.Workbook()
    ws = wb.active
    with pytest.raises(BlastpStoreSchemaError, match="BLASTP_B5_SCHEMA_HOLD"):
        _existing_b5_keys(ws)
