...
def test_session_manifest_header():
    ...
    assert needle in _read("docs/BUNDLE_CAPABILITIES.md"), \
        f"docs/BUNDLE_CAPABILITIES.md header is not '{needle}'"
...
def test_readme_bundle_version():
    mentions = re.findall(r"sapote-mamey-v(\d+(?:\.\d+)*[a-z]?)", _read("README.md"))
    assert mentions, "README.md has no sapote-mamey-v<ver> mention"
    assert all(m == BUNDLE for m in mentions), \
        f"README.md states {set(mentions)}, expected all == {BUNDLE}"
...
def test_generated_bootstrap_docs_current_via_sync():
    ...
    the docs/BOOTSTRAP_FILE_AUDIT.md-stale-at-v9.7.153 miss in the v9.7.154 cut. Skips
