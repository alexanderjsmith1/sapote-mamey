...
def _gate_tools():
    ...
    for f in sources:
        ...
        if GATE_NAME.search(b):
            if b in seen:
                left = seen[b].relative_to(TOOLS)
                right = f.relative_to(TOOLS)
                raise AssertionError(
                    f"duplicate gate tool stem {b!r}: {left} and {right}; "
                    "the stem-keyed registry cannot distinguish them"
                )
            seen[b] = f
            out.append(b)
    return out
...
def test_nested_gate_tools_are_discovered_and_non_gates_stay_excluded(tmp_path, monkeypatch):
    nested = tmp_path / "tools" / "nested"
    nested.mkdir(parents=True)
    (nested / "check_fixture.py").write_text("\"\"\"fixture gate\"\"\"\n", encoding="utf-8")
    (nested / "build_fixture.py").write_text("\"\"\"fixture builder\"\"\"\n", encoding="utf-8")
    cache = nested / "__pycache__"
    cache.mkdir()
    (cache / "check_cached.py").write_text("", encoding="utf-8")
    monkeypatch.setattr(sys.modules[__name__], "TOOLS", tmp_path / "tools")
    assert _gate_tools() == ["check_fixture"]


def test_recursive_gate_discovery_refuses_duplicate_stems(tmp_path, monkeypatch):
    for dirname in ("left", "right"):
        nested = tmp_path / "tools" / dirname
        nested.mkdir(parents=True)
        (nested / "check_fixture.py").write_text("\"\"\"fixture gate\"\"\"\n", encoding="utf-8")
    monkeypatch.setattr(sys.modules[__name__], "TOOLS", tmp_path / "tools")
    with pytest.raises(AssertionError, match="duplicate gate tool stem.*check_fixture"):
        _gate_tools()


def test_recursive_gate_discovery_does_not_admit_symlinked_files(tmp_path, monkeypatch):
    tools = tmp_path / "tools"
    outside = tmp_path / "outside"
    tools.mkdir()
    outside.mkdir()
    target = outside / "check_external.py"
    target.write_text("\"\"\"outside fixture\"\"\"\n", encoding="utf-8")
    (tools / "check_external.py").symlink_to(target)
    external_dir = outside / "nested"
    external_dir.mkdir()
    (external_dir / "check_external_dir.py").write_text("", encoding="utf-8")
    (tools / "linked_dir").symlink_to(external_dir, target_is_directory=True)
    monkeypatch.setattr(sys.modules[__name__], "TOOLS", tools)
    assert _gate_tools() == []
