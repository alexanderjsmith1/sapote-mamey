...
def test_protocol_file_ships():
    # The canonical game rules live in games/; docs/ has a pointer
    p = ROOT / "games" / "BUNNY_HOP_AUDIT_GAME.md"
    assert p.exists(), "Bunny Hop protocol must ship at games/BUNNY_HOP_AUDIT_GAME.md"
    ...
    assert "games/" in pointer.read_text(encoding="utf-8")
...
def test_trigger_documented_in_chatgpt_contract():
    text = (ROOT / "CHATGPT_START_HERE.md").read_text(encoding="utf-8")
    assert "bunny hop" in text.lower()
    assert "BUNNY_HOP_AUDIT_GAME.md" in text
...
def test_chatgpt_init_runs_and_reports_contract():
    ...
    assert "CHATGPT_START_HERE.md" in out.stdout
