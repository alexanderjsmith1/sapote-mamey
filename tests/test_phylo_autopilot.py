...
def test_parse_blast_genus_holds_near_tied_different_genus():
    txt = ("Q1\tMicromonospora echinospora strain T\t99.20\t1400\n"
           "Q1\tActinomadura madurae strain T\t99.00\t1395\n"
           "Q2\tStreptomyces griseus strain T\t99.20\t1400\n"
           "Q2\tNocardia farcinica strain T\t97.00\t1400\n")
    got = AP.parse_blast_genus(txt)
    assert got["Q1"]["routing_state"] == "GENUS_CONFLICT"
    assert got["Q1"]["alternate_genera"] == ["Actinomadura"]
    assert got["Q2"]["routing_state"] == "TOP_HIT_ONLY"


def test_genus_conflict_does_not_enter_a_reference_group():
    assigned = {"Q1": {"genus": "Micromonospora", "pident": 99.2, "aln_len": "1400",
                       "title": "t", "routing_state": "GENUS_CONFLICT",
                       "alternate_genera": ["Actinomadura"]}}
    assert AP.genera_for_reference(assigned) == {}
...
def test_write_routing_table(tmp_path):
    ...
    assert body[0].startswith("query\ttophit_genus") and "routing_state" in body[0]
...
def test_run_16s_passes_bootstrap_to_phylo_place(tmp_path, monkeypatch):
    ...
    assert "--fragmentary" not in seen["cmd"]
    qc = (tmp_path / "o" / "query_qc.tsv").read_text()
    assert "AS-1\t1400\t0.000000\t0\tfalse\tADMITTED" in qc


def test_run_16s_selects_fragment_alignment_for_partial_query(tmp_path, monkeypatch):
    q = _write(tmp_path, "q.fasta", [("AS-1", "ACGT" * 200)])
    monkeypatch.setattr(AP, "assign_genus", lambda *a, **k: {
        "AS-1": {"genus": "Micromonospora", "pident": 99.0, "aln_len": "800", "title": "t"}})
    monkeypatch.setattr(AP, "build_reference_fasta", lambda *a, **k: 5)
    seen = {}
    ...
    monkeypatch.setattr(AP.subprocess, "run", fake_run)
    rc = AP.main(["run-16s", "--query", q, "--db", "DB", "--group", "rare_genera",
                  "--outdir", str(tmp_path / "o"), "--approved-by", "tester"])
    assert rc == 0
    assert "--fragmentary" in seen["cmd"]
    assert "AS-1\t800\t0.000000\t0\ttrue\tADMITTED" in (tmp_path / "o" / "query_qc.tsv").read_text()


def test_run_16s_holds_too_short_or_invalid_query_before_blast(tmp_path, monkeypatch, capsys):
    q = _write(tmp_path, "q.fasta", [("AS-1", "ACGT" * 20 + "!")])
    called = {"blast": False}

    def fail_if_called(*a, **k):
        called["blast"] = True
        raise AssertionError("BLAST must not run for a held query")
    monkeypatch.setattr(AP, "assign_genus", fail_if_called)
    rc = AP.main(["run-16s", "--query", q, "--db", "DB", "--group", "rare_genera",
                  "--outdir", str(tmp_path / "o"), "--dry-run"])
    assert rc == 2
    assert not called["blast"]
    qc = (tmp_path / "o" / "query_qc.tsv").read_text()
    assert "QUERY_UNSUPPORTED_SYMBOLS|QUERY_TOO_SHORT" in qc
    assert "refusing 1 query record" in capsys.readouterr().err


def test_query_qc_warns_without_silently_trimming():
    rows = AP._p16.screen_query_records([("q", "N" * 30 + "ACGT" * 620)])
    assert rows[0]["length"] == 2510
    assert rows[0]["admission_state"] == "ADMITTED"
    assert rows[0]["warnings"] == ["POSSIBLE_NON_16S_OR_UNTRIMMED", "LONG_TERMINAL_N_RUN"]
