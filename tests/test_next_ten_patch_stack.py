...
def test_llm_handoff_receipt_scans_start_files(tmp_path):
    ...
    (tmp_path / "CLAUDE.md").write_text("# Claude start\n")
...
def test_release_qa_runs_legacy_and_llm_gates(tmp_path):
    ...
    bundle.mkdir()
    (bundle / "AGENTS.md").write_text(f"# ChatGPT\n\n{HANDSHAKE}\n")
    (bundle / "CLAUDE.md").write_text("# Claude\n")
...
def test_llm_handoff_excludes_pycache_and_bytecode(tmp_path):
    (tmp_path / "AGENTS.md").write_text(f"# ChatGPT\n\n{HANDSHAKE}\n")
    (tmp_path / "AGENTS.md").write_text(f"# Claude\n\n{HANDSHAKE}\n")
...
def test_release_qa_receipt_subpaths_are_relative(tmp_path):
    ...
    bundle.mkdir()
    (bundle / "AGENTS.md").write_text(f"# ChatGPT\n\n{HANDSHAKE}\n")
    (bundle / "AGENTS.md").write_text(f"# Claude\n\n{HANDSHAKE}\n")
    receipt = run_release_qa(
        bundle,
        bundle / "release_qa",
        create_default_legacy_matrix=True,
