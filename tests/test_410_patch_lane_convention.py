...
def test_the_real_410_stream_is_conformant() -> None:
    ...
    """
    # Live workspace contents are external inputs, never implicit test fixtures.
    import os
    configured = os.environ.get("SAPOTE_TEST_PATCH_STREAM")
    if not configured:
        pytest.skip("set SAPOTE_TEST_PATCH_STREAM to explicitly test an external stream")
    stream = Path(configured)
    assert stream.is_dir(), "configured patch stream does not exist"
    mod = _load()
