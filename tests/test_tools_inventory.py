...
def test_every_tool_is_indexed():
    ...
    # v9.7.419: was gen.TOOLS.iterdir() — depth 1 only, so the 13 nested tools could
    # never appear in `on_disk`, `missing` was always empty for them, and this guard passed
    # while the inventory it polices was 13 tools short. Uses the generator's own discovery
    # so the test and the tool can never disagree about scope again.
    on_disk = {p.relative_to(gen.TOOLS).as_posix() for p in gen.tool_files()}
...
def test_manifest_block_is_present_and_current():
    ...
    assert gen.render_manifest_block(gen.collect()) in text, \
        "docs/BUNDLE_CAPABILITIES.md tools block is stale — run tools/gen_tools_inventory.py"
...
def test_connection_audit_uses_real_wiring_and_excludes_generated_self_certification(tmp_path):
    ...
    _write(
        tmp_path / "docs/BUNDLE_CAPABILITIES.md",
