...
TRIAGE_HEADER = (
    "BGC_ID,Contig,Node_ID,Products,Boundary,AB_auto,AF_auto,Novelty_auto,"
    "CCTT_triggers,Lead_tier_auto,KCB_top,Depth_floor,Standing_rule,Primary_metab_flag\n"
)
TRIAGE_ROWS = (
    "BGC001,NODE_1_length_50000,NODE_1,NRPS;thioamide-NRP,Interior,70.0,28.0,45.0,"
    "T43-THA_thioamide,High,enteromycin,,,\n"
    "BGC002,NODE_2_length_40000,NODE_2,saccharide,Edge,63.0,30.0,50.0,"
    ",Inventory,natronosporangium,,saccharide-exclusion,\n"
    "BGC003,NODE_3_length_30000,NODE_3,RiPP;triceptide,Interior,55.0,38.0,56.0,"
...
def _make_pkg(tmp_path: Path, strain="TEST-STRAIN") -> Path:
    pkg = tmp_path / strain
    pkg.mkdir()
    (pkg / f"{strain}_4_triage_board.csv").write_text(TRIAGE_HEADER + TRIAGE_ROWS)
    return pkg
