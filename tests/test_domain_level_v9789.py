...
# --- command smoke + fallback (admitted full-locus synthetic fixture) ---



def test_command_writes_outputs_limited_mode(tmp_path, public_domain_reference_zip):
    import subprocess, glob
    out = str(tmp_path / "run")
    env = dict(os.environ, PYTHONPATH=str(BUNDLE_ROOT))
    subprocess.run(
        [sys.executable, str(BUNDLE_ROOT / "mamey_run.py"), "run", "--strain", "DLSMOKE", "--display", "ref",
         "--input-zip", str(public_domain_reference_zip), "--taxonomy", "Streptomyces", "--source", "test",
