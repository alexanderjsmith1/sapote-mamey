...
@pytest.mark.parametrize("code,text", [
    ...
    (5, "no tests ran"), (4, "999 passed"), (0, ""),
    (1, "1 passed"), (0, "1 failed"),
])
def test_subset_rejects_execution_failure_even_with_summary_text(tmp_path, monkeypatch, code, text):
    from types import SimpleNamespace
    monkeypatch.setattr(grm.subprocess, "run", lambda *a, **k:
                        SimpleNamespace(returncode=code, stdout=text, stderr=""))
    with pytest.raises(SystemExit):
        grm._run_pytest_subset([tmp_path / "test_consumer.py"], tmp_path)


def test_usage_error_cannot_change_manifest_or_converge(tmp_path, monkeypatch):
    from types import SimpleNamespace
    manifest = _manifest(tmp_path)
    before = manifest.read_bytes()
    monkeypatch.setattr(grm, "discover_manifest_reading_tests", lambda root: [root / "test_consumer.py"])
    monkeypatch.setattr(grm.subprocess, "run", lambda *a, **k:
                        SimpleNamespace(returncode=4, stdout="", stderr="usage error"))
    with pytest.raises(SystemExit, match="execution failed"):
        grm.run_fixed_point(tmp_path, seed_passed=500, seed_skipped=10, provenance="test")
