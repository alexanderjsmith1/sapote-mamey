...
def test_every_subcommand_is_catalogued_and_grouped():
    ...
    canonical = mod._subparsers()
    grouped = {n for _, _, names in mod.GROUPS for n in names}
    ungrouped = sorted(set(canonical) - grouped)
    assert not ungrouped, f"add these to GROUPS in tools/gen_command_catalog.py: {ungrouped}"


def test_aliases_fold_into_canonical_rows_with_descriptions():
    text = DOC.read_text(encoding="utf-8")
    assert "| `discover` (alias: `discovery`) | Orient in a workspace" in text
    assert "| `genus-appendix` (alias: `af-ab-appendix`) | Antifungal/antibacterial" in text
...
def test_markdown_table_escapes_pipes_inside_help_text():
    import re
    text = DOC.read_text(encoding="utf-8")
    row = [l for l in text.splitlines() if l.startswith("| `figures` |")][0]
    assert "diagram \\| atlas \\| ani \\| gcf-network \\| clinker" in row
    assert len(re.findall(r"(?<!\\)\|", row)) == 3, "help text created extra Markdown table columns"
